...
Il vecchio handoff/mapping degli ID non è reperibile. Il registro seguente è quindi una **nuova baseline rebaselined**, proprietaria del piano al **2026-09-17**: integra i contratti sopravvissuti e le evidenze del tree, ma non finge equivalenza storica con la semantica originaria dei finding. La baseline comune resta `d9591ac7`; la remediation conclusiva è raccolta nel candidato `4a729d50` della [PR #50](https://github.com/Fubeo/Fub/pull/50). `VERIFIED_LOCAL` indica che implementazione, prova locale e documentazione sono coerenti nel candidato; `ACCEPTED_RISK` indica un limite reale esplicitato nella riga. La colonna CI resta `pending` finché i required checks del commit finale non sono tutti verdi.
### Sicurezza
...
| ID | Specifica rebaselined | Stato | Patch | Test/guardia locale | Docs | Rischio residuo | CI |
|---|---|---|---|---|---|---|---|
| UI-001 | `apps/client/bench/a11y.mjs` esegue axe completo su ogni scena e luce; nessun passaggio è valido su zero elementi e la matrice ha 42/42 scene, 48.612 elementi, 0 failure e 0 debito dichiarato. | `VERIFIED_LOCAL` | `d9591ac7` | `apps/client/bench/a11y.mjs` `axe.run`; full axe locale 42/42 scene, 48.612 elementi | `docs/development/testing-and-quality.md` § Visuale e accessibilità | I 388 `incomplete` sono riportati e la resa assistiva reale resta dipendente da browser/Tauri e tecnologia assistiva. | `pending` |
...
| ID | Specifica rebaselined | Stato | Patch | Test/guardia locale | Docs | Rischio residuo | CI |
|---|---|---|---|---|---|---|---|
| ABI-001 | ABI `0.1.2` accetta solo versioni canoniche complete: stessa major, minor guest non superiore; forme parziali/future/overflow sono rifiutate. | `VERIFIED_LOCAL` | `d9591ac7` | `crates/fub-abi/src/traits.rs` canonical version test; Rust locale 2171 pass/3 ignored | `docs/reference/abi-and-wit.md` § Compatibilità | La regola dipende dall’additività effettiva del WIT; incompatibilità semantiche possono restare. | `pending` |
...
Il registro contiene esattamente questi ID unici: 4 SEC + 11 DATA + 9 UI + 7 ARCH + 3 ABI + 3 WASM + 2 BUILD + 1 CI + 13 DOC + 2 THEME + 1 PLAN = **56**. Sul candidato `4a729d50` le verifiche locali aggregate sono: Rust **2171 pass/3 ignored** in 168 suite; client **95 file/1380 test**; axe completo **42/42 scene**, **48.612 elementi**, 0 failure, 0 debito dichiarato e 388 incomplete; contention release con 0 errori di lettura; `npm audit` **0**; fmt/check/clippy e build client verdi. Questi risultati non sostituiscono i required checks remoti dello stesso commit e non chiudono da soli G14.
