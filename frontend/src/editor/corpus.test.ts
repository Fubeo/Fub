...
import { parseWikilinkInner } from "../rules/sintassi";
import corpus from "../__fixtures__/corpus-sintassi.json";
...
const casi = corpus as CasoCorpus[];
...
const DIVERGENZE: Record<string, string> = {
  ...
  // `relazione: "[[Nota]]"` la live preview disegna un wikilink cliccabile,
  // mentre per il modello quello è il valore di una proprietà e non un link del
...
function statoDi(doc: string): EditorState {
...
/// dagli `hide`.
function decoraTutteAttive(doc: string): LiveDeco[] {
  const state = statoDi(doc);
  const attive = new Set<number>();
  for (let n = 1; n <= state.doc.lines; n++) attive.add(n);
  return computeDecorations(state, attive);
}
...
function confronta(caso: CasoCorpus, famiglia: string, atteso: unknown, avuto: unknown): void {
  const chiave = `${caso.nome}/${famiglia}`;
  const ragione = DIVERGENZE[chiave];
  if (ragione) {
    // Una divergenza dichiarata che **non** si presenta più è rossa: è così che
    // la lista non diventa un ricordo.
    expect(avuto, `«${chiave}» non diverge più: togli la riga da DIVERGENZE`).not.toEqual(atteso);
    return;
  }
  expect(avuto, `«${chiave}»`).toEqual(atteso);
}

describe("il corpus: le due passate dicono la stessa cosa", () => {
  it("la fixture porta abbastanza casi da poter divergere", () => {
    expect(casi.length).toBeGreaterThan(30);
    const conta = (f: (c: CasoCorpus) => unknown[]) =>
      casi.reduce((n, c) => n + f(c).length, 0);
    expect(conta((c) => c.tag)).toBeGreaterThan(2);
    expect(conta((c) => c.wikilink)).toBeGreaterThan(2);
    expect(conta((c) => c.task)).toBeGreaterThan(2);
  });
  ...
  for (const caso of casi) {
    it(`«${caso.nome}»`, () => {
      const attive = decoraTutteAttive(caso.source);

      // --- i tag: confini esatti, e sono quelli della regola del contratto ---
      confronta(
        caso,
        "tag",
        caso.tag.map((t) => [t.name, t.from, t.to]),
        di(attive, "tag").map((d) => [d.data, d.from, d.to]),
      );
      ...
      confronta(
        ...
        caso.wikilink.map((w) => [w.page, w.from + (w.embed ? 3 : 2), w.to - 2]),
        di(attive, "wikilink").map((d) => [
      ...
      const inattive = computeDecorations(statoDi(caso.source), new Set());
      confronta(
        caso,
        "task",
        caso.task.map((t) => [t.from - 1, t.to + 1]),
        di(inattive, "checkbox").map((d) => [d.from, d.to]),
      );
    });
  }
});
