...
// forma: una `Vita` per finestrella aperta, e chiudere è chiuderla.
//
...
describe("il selettore di icona", () => {
  ...
  it("riaprirlo non lascia appesa la finestrella di prima", () => {
    const scelte: (string | null)[] = [];
    pickIcon(clic(), (i) => scelte.push(i));
    pickIcon(clic(), (i) => scelte.push(i));
    ...
    // il tab di una superficie che non c'era più.
    expect(tab().defaultPrevented).toBe(false);
  });
