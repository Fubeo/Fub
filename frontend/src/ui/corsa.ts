...
// quella è la stessa promessa ripetuta, e in `frontend/src/` era già scritta a
...
//
// La risposta è che **il controllo non sia una riga che si scrive**: `atteso` è
// l'unico modo in cui il corpo di un giro ottiene il risultato di un'attesa, e
// se nel frattempo è cominciato un giro più nuovo `atteso` non torna affatto —
...
// marchio**: `atteso` arriva come parametro del corpo, cioè per averlo bisogna
...
// stessa domanda e hanno risposte **opposte**. Con una `Corsa` il lavoro scaduto
...
// ha in mano una `Corsa` non può accodare per sbaglio, e chi ha una `Coda` non
...
// `await atteso(promessa.catch(…))` — e non un `try` attorno all'`atteso`: così
...
//
// A guardare che nessuno scavalchi è `.github/scripts/check-corse.mjs`.
...
const SCADUTO = Symbol("corsa: giro scaduto");

/// Come il corpo di un giro ottiene il risultato di un'attesa.
///
/// Si usa **al posto di `await`**: `const x = await atteso(promessa)`. Se il
...
export type Atteso = <T>(promessa: Promise<T>) => Promise<T>;
...
export class Corsa {
  ...
  async ultimo<T>(corpo: (atteso: Atteso) => Promise<T>): Promise<T | undefined> {
    const mio = ++this.#ultimo;
    const atteso = async <V,>(promessa: Promise<V>): Promise<V> => {
      let valore: V;
      try {
        valore = await promessa;
      } catch (e) {
        ...
        if (mio !== this.#ultimo) throw SCADUTO;
        throw e;
      }
      if (mio !== this.#ultimo) throw SCADUTO;
      return valore;
    ...
    try {
      return await corpo(atteso);
    } catch (e) {
      if (e === SCADUTO) return undefined;
      throw e;
    }
  }
  ...
  /// finto, ed è il motivo per cui `Corsa` è un oggetto con due metodi invece di
...
/// ha toccato solo lui.
///
/// Era già scritta a mano in `panels/document.ts` come una `let coda:
/// Promise<void>` con la sua catena di `.then`, e la ragione per cui adesso è un
...
export class Coda {
  ...
  ///
  /// Uno sbaglio non ferma la coda — `#ultimo` riparte dalla catena catturata,
  ...
  accoda<T>(lavoro: () => Promise<T>): Promise<T> {
    // `#ultimo` non rigetta mai — sotto lo si tiene già disinnescato — quindi
    ...
    const mio = this.#ultimo.then(lavoro);
