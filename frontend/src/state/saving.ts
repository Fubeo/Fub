...
// provandolo: `panels/document.ts` monta editor, linguetta e riquadri, e importarlo
...
export type Outcome = "ok" | "in_corso" | "fallito" | "conflitto";
...
/// stata scritto diceva «quattro», ed era vera; poi è arrivato `conflitto` e la
...
export function stateOf(buf: { dirty: boolean; result: Outcome } | undefined): SaveState | null {
  if (!buf) return null;
  ...
  if (buf.result === "conflitto") return "conflitto";
  if (buf.result === "fallito") return "fallito";
  if (buf.result === "in_corso") return "in_corso";
...
export type UnderChange = "muto" | "eco" | "altra_app" | "riscrittura";
...
/// **seconda metà** del conto degli echoes, e la prima è `writeCountingEcho` qui
...
/// compariva. Il commento su `Buffer.echoes` dichiarava già la regola giusta — «il
/// primo evento non-watcher su quel documento lo consuma» — ed era il codice a
/// non applicarla.
///
/// `source` risponde **prima** del contatore e non lo tocca mai, ed è
...
/// 0010).
export function consumeUnderChange(
  buf: { dirty: boolean; echoes: number } | undefined,
  source: Origin,
): UnderChange {
  if (!buf) return "muto";
  if (source.actor.kind === "watcher") return buf.dirty ? "altra_app" : "muto";
  ...
  if (source.actor.kind !== "user" || source.batch !== null) {
    return buf.dirty ? "riscrittura" : "muto";
  }
  const ours = buf.echoes > 0;
  if (ours) buf.echoes -= 1;
  if (!buf.dirty) return "muto";
  return ours ? "eco" : "riscrittura";
}
...
/// `consumaUnderChange` qui sopra: il conto vive tutto in questo file, e fuori
/// di qui le righe che lo muovono sono **zero** [conta: echoes-fuori-dal-padrone].
...
/// nessuna.
export async function writeCountingEcho<T>(
  buf: { echoes: number },
  write: () => Promise<T>,
): Promise<T> {
  buf.echoes += 1;
