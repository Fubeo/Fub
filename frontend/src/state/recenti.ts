...
// filtro corrente, tab attiva* — «dove eri rimasto». Una cronologia è un'altra
...
import { documentiEsistenti, impostazioni } from "../host/query";
import { onEvent } from "./kernel";
import { leggiStato, on, scriviStato } from "./store";
...
export const CHIAVE_CRONOLOGIA = "history.enabled";
...
export function conInCima(lista: string[], voce: string, max = QUANTE): string[] {
  return [voce, ...lista.filter((d) => d !== voce)].slice(0, max);
}
...
export async function noteRecentiEsistenti(): Promise<string[]> {
  if (aperture.length === 0) return [];
  const vive = await documentiEsistenti(aperture);
  return aperture.filter((d) => vive.has(d));
}
...
export function ricordaRicerca(testo: string): void {
  const q = testo.trim();
  if (q === "") return;
  ricerche = conInCima(ricerche, q);
  metti_via();
}
...
function metti_via(): void {
  if (!accesa) return;
  scriviStato(
    CHIAVE_STATO,
    aperture.length === 0 && ricerche.length === 0 ? null : { note: aperture, ricerche },
  );
}
...
/// path.
function leggiListe(v: unknown): { note: string[]; ricerche: string[] } {
  const vuote = { note: [], ricerche: [] };
  if (!v || typeof v !== "object") return vuote;
  const o = v as Record<string, unknown>;
  const lista = (x: unknown): string[] =>
    Array.isArray(x) ? x.filter((e): e is string => typeof e === "string").slice(0, QUANTE) : [];
  return { note: lista(o.note), ricerche: lista(o.ricerche) };
}
...
async function rileggiInterruttore(): Promise<void> {
  const prima = accesa;
  try {
    ...
    const entry = (await impostazioni()).find((e) => e.spec.key === CHIAVE_CRONOLOGIA);
    if (!entry) return;
    accesa = entry.value !== false;
  } catch {
  ...
  if (prima && !accesa) dimenticaTutto();
}
...
export async function caricaCronologia(): Promise<void> {
  await rileggiInterruttore();
  const salvato = await leggiStato<unknown>(CHIAVE_STATO);
  if (!accesa) {
    ...
    if (salvato !== null) dimenticaTutto();
    return;
  }
  const liste = leggiListe(salvato);
  aperture = liste.note;
  ricerche = liste.ricerche;
}
...
export function ricordaLeAperture(): void {
  on("active-doc", (doc) => {
    if (doc === null) return;
    aperture = conInCima(aperture, doc);
    metti_via();
  });
  ...
  // quindi si rilegge.
  onEvent("setting_changed", () => void rileggiInterruttore());
}
