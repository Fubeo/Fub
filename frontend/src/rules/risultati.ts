...
export interface RigaDiRisultato {
  doc: string;
...
export function righeDaMostrare(hits: DocumentMatch[]): RigaDiRisultato[] {
  const righe: RigaDiRisultato[] = [];
  for (const hit of hits) {
    const occorrenze = hit.occurrences ?? [];
    righe.push({
      doc: hit.doc,
      byteOffset: occorrenze[0]?.span.start,
      snippet: hit.snippet,
      highlights: hit.highlights,
    });
    occorrenze.slice(1).forEach((posizione, i) => {
      righe.push({ doc: hit.doc, byteOffset: posizione.span.start, occorrenza: i + 2 });
    });
  }
  return righe;
}
