...
import { nomeDaCercato } from "./nome-cercato";
...
describe("nomeDaCercato", () => {
  ...
  it("solo spazi, o solo caratteri vietati, non propongono niente", () => {
    ...
    expect(nomeDaCercato("   ")).toBeNull();
    expect(nomeDaCercato("///")).toBeNull();
    expect(nomeDaCercato("")).toBeNull();
  });

  it("tre righe incollate diventano un nome leggibile", () => {
    const lungo = nomeDaCercato("x".repeat(300));
    expect(lungo).not.toBeNull();
    expect(lungo?.length).toBe(80);
  });
});
