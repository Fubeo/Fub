...
//    `struttura.css` già ascolta: si fotografa lo stato d'arrivo e mai un
...
export const ISTANTE = new Date("2026-08-19T09:30:00+02:00");
...
export const FINESTRA = { width: 1280, height: 800 };
...
/// corsa non parte.
export async function apriIlPalco({ silenzioso = true } = {}) {
  const server = await createServer({
    configFile: qui("../vite.banco.config.ts"),
    logLevel: silenzioso ? "warn" : "info",
  });
  await server.listen();
...
export async function apriUnaPagina(browser, luce) {
...
export async function preparaScena(page, scena, luce, base, indirizzo) {
  await page.goto(indirizzo(base, scena, luce), { waitUntil: "load" });
  await page.waitForFunction(() => document.documentElement.dataset.banco === "pronto", null, {
    timeout: 30_000,
  });
  await page.evaluate(() => document.fonts.ready);
  await scena.prepara(page);
  await page.evaluate(() => document.fonts.ready);
  await page.addStyleTag({ content: QUIETE });
...
export async function scattoFermo(page, tentativi = 4) {
  let prima = await page.screenshot();
  for (let i = 0; i < tentativi; i += 1) {
    await page.evaluate(
      () => new Promise((r) => setTimeout(() => requestAnimationFrame(r), 250)),
    );
    const dopo = await page.screenshot();
    if (prima.equals(dopo)) return dopo;
    prima = dopo;
  }
