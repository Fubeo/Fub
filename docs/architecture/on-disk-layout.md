...
**Dentro un vault, ciò che FubMD scrive sta in una radice sola: `.fubmd/`.** La
profondità dice la classe:

| Dove | Classe | Cosa vuol dire |
|---|---|---|
| `<vault>/.fubmd/` | **autorevole** | perso, non si ricostruisce da niente. Chi non riesce a leggerlo **non lo sovrascrive** |
| `<vault>/.fubmd/data/` | **derivato** | si butta e si rifà dal vault. Chi non riesce a leggerlo lo rifà, e non avvisa nessuno |

Fuori dalla radice resta `<vault>/.trash/`, che non è roba di FubMD: è il
...
`FUBMD_CONFIG_DIR`, o il modo portable accanto all'eseguibile). Ci sta ciò che
...
erano **due**: `.fubmd/` per l'autorevole e `.fubmd-data/` per il derivato.
...
|---|---|
| `.fubmd-data/` | `.fubmd/data/` |
| `.fubmd-data/plugins/<id>/` | `.fubmd/data/plugins/<id>/` |
| `.fubmd-data/entries.json` | `.fubmd/data/entries.json` |
| `.fubmd-data/trash/` | `.fubmd/data/trash/` |
