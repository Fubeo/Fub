...
| funzione su un'interfaccia **importata** dal plugin | una funzione **nuova** | cambiare parametri o risultato di una esistente |
| funzione su un'interfaccia **esportata** dal plugin | — | una funzione **nuova** è un'**obbligazione**, non un'aggiunta: vedi sotto. E cambiare parametri o risultato di una esistente |
...
**Da che parte sta l'interfaccia cambia il verso della riga «funzione nuova»**, e
per un pezzo di storia questa tabella aveva una casella sola. Su ciò che il
plugin **importa** l'aggiunta è vera davvero: un componente già compilato non
chiama la funzione nuova, non se ne accorge, continua a girare — è il caso della
[decisione 0013](../decisions/0013-elenco-delle-capacita.md). Su ciò che il
plugin **esporta** — le interfacce che il `world` elenca fra i suoi export, cioè
quelle che il plugin deve *implementare* — una funzione in più non è qualcosa che
il vecchio può ignorare: è qualcosa che deve **fornire**. Un componente compilato
contro `fub:abi@0.1.0` esporta le funzioni di allora, e un world che ne pretende
una in più è un world che quel componente non soddisfa: non si instanzia affatto.
È una major travestita da riga in coda, e la
[decisione 0102](../decisions/0102-i-byte-non-stanno-nel-record.md) l'ha trovata
dopo che ne erano già passate due — `index::up-to-date` e `view::interests`, che
sono in `OBBLIGAZIONI_NOTE` dentro `wit_additivity.rs`, ognuna con la sua
ragione.
...
| [decisione 0094](../decisions/0094-un-tetto-che-si-fa-sentire.md) | **`host-env.random-bytes` passa da `list<u8>` a `result<list<u8>, plugin-error>`** (§23.12): è un **tipo di ritorno cambiato**, la rottura più semplice delle venti che `wit_additivity` elenca, e il più piccolo dei tre ritagli di firma di questa seduta — nessun tipo nuovo entra, perché `bad-args` e `permission-denied` erano già le due frasi giuste. La lista nuda diceva **tre** cose con la stessa forma: i byte chiesti, il troncamento sopra il tetto dell'host, e l'assenza della capacità `env`. La [0039](../decisions/0039-il-locale-e-il-caso.md) confondeva le prime due deliberatamente («chi chiede di più riceve mille byte e **non** un errore»); la terza non l'aveva nominata nessuno, ed è la più grave, perché una politica travestita da dato è la famiglia della [0013](../decisions/0013-elenco-delle-capacita.md) e della [0021](../decisions/0021-il-confine.md), non quella di un tetto. Ciò che **non** entra nel confine è il tetto: un numero pubblicato è una promessa congelata, e quello che il contratto deve garantire è che superarlo si dica — la forma dell'`overflow` della [0034](../decisions/0034-il-freno-e-il-raggruppamento.md), dove si pubblica la perdita e non la soglia. La via additiva (`max-random-bytes: func() -> u32`) è stata scartata a verbale per la ragione che la voce stessa dichiarava: chi non controlla la lunghezza non chiederebbe nemmeno il massimo |
| [decisione 0101](../decisions/0101-una-voce-non-e-un-passo.md) | **`host-commands.undo-last` passa da `result<option<text>, plugin-error>` a `result<option<undone>, plugin-error>`** (§23.14): un tipo di ritorno cambiato, la stessa rottura della [0094](../decisions/0094-un-tetto-che-si-fa-sentire.md). Il `text` nudo era l'etichetta della voce e basta, e diceva **una** cosa dove ce n'erano tre, perché una voce di undo non è un passo ma una **lista**: che l'operazione annullata fosse già riuscita a metà per conto suo, e che l'annullamento stesso si sia fermato al passo caduto lasciando applicati quelli prima e non provati quelli dopo. Le risposte passano da due a quattro, e la terza — *niente è cambiato* — resta un `err`, così la promessa che la firma faceva già («un annullamento può fallire come qualunque scrittura») vale ancora alla lettera per chi la usava. **Questo ritaglio NON tocca `0.1.0.wit`, e non è una svista**: `undo-last` è nata con la [0045](../decisions/0045-l-undo-ha-due-pile.md), cioè dopo che la linea di base è stata tagliata — nello snapshot l'interfaccia `host-commands` non ha `undo`, e nemmeno `command-outcome` ha il campo. Vale la ragione della [0049](../decisions/0049-una-posizione-dentro-un-documento.md): ritipare una funzione mai pubblicata non rompe nessuna promessa, `wit_additivity` è verde **con ragione**, e scrivere nel file della linea di base una firma che non c'era falsificherebbe cosa è stato pubblicato. La rottura resta reale per chi compila contro l'`abi.wit` di oggi. Il campo `command-outcome.partial` è invece additivo **davvero** — in coda a un record che nella linea di base ha due campi soli — insieme ai tipi nuovi `partial` e `failure` |
| [decisione 0102](../decisions/0102-i-byte-non-stanno-nel-record.md) | **`import-source.bytes` diventa `content: source-content`, e `export-artifact.bytes` diventa `content: artifact-content`** (§23.6): due campi di record ritipati, la prima delle venti rotture che `wit_additivity` elenca, più i tipi nuovi `source-handle`, `artifact-handle`, `streamed-source`, `source-content`, `artifact-content` e le due interfacce `host-transfer-read` e `host-transfer-write`. A differenza della [0049](../decisions/0049-una-posizione-dentro-un-documento.md) e della [0101](../decisions/0101-una-voce-non-e-un-passo.md), **qui `0.1.0.wit` si tocca davvero**: i due campi c'erano già quando la linea di base è stata tagliata, quindi non si sta ritipando qualcosa che non era mai stato pubblicato — si sta cambiando qualcosa che c'era, ed è la ragione per cui si fa prima del freeze M4 e non dopo. Ciò che compra: i byte di un trasferimento smettono di stare **dentro** il record e passano da una chiave che l'host timbra, nei due versi. Ciò che **non** costa: nessuna capacità filesystem, in nessuno dei due versi — la conclusione della [0006](../decisions/0006-import-export-come-trait.md) vale identica dopo. Restare in memoria non è vietato ed è il caso comune (`source-content.bytes` è ancora lì): cambia che la scelta si dichiara invece di essere l'unica. Il `read-chunk` di 256 KiB **non** entra nel confine, per la ragione della [0094](../decisions/0094-un-tetto-che-si-fa-sentire.md): un numero pubblicato è una promessa congelata |
| [decisione 0132](../decisions/0132-un-rifiuto-non-e-una-frase.md) | **`format-error.unsupported` passa da `string` a un record `format-error-unsupported { format, got }`** (§24.3): un caso di `variant` **ritipato**, la seconda delle venti rotture che `wit_additivity` elenca, più il tipo nuovo nella stessa interfaccia. Come nella [0102](../decisions/0102-i-byte-non-stanno-nel-record.md) — e a differenza della [0049](../decisions/0049-una-posizione-dentro-un-documento.md) e della [0101](../decisions/0101-una-voce-non-e-un-passo.md) — **`0.1.0.wit` si tocca davvero**: quel caso c'era già quando la linea di base è stata tagliata. Pesa più di quanto la sua riga faccia pensare, perché `format-error` è il tipo d'errore di cinque funzioni che il plugin **esporta** (`format.parse`, `format.render-html`, `format.serialize`, `syntax.apply`, `custom-render.render`): non è una riga che il vecchio possa ignorare, è la forma con cui deve rispondere. La `string` era prosa scritta da chi implementa il provider, cioè l'unico errore del contratto che arrivava a uno schermo senza che nessuno potesse comporne la frase; adesso porta i **due dati** da cui la frase si compone, e a comporla è chi la mostra. La via additiva — un caso in coda, lasciando `unsupported(string)` dov'è — è stata scartata a verbale per la ragione della [0049](../decisions/0049-una-posizione-dentro-un-documento.md) e della [0089](../decisions/0089-da-cosa-e-partita-una-scrittura.md): lascerebbe per sempre **due modi di rifiutare una sorgente, di cui uno intraducibile**, e chi scrive un provider sceglierebbe il più corto |
