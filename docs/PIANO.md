...
| Capacità dell'`HostApi` ([0013](decisions/0013-elenco-delle-capacita.md)) | **Elenco chiuso alla sottrazione**: ventidue metodi alla chiusura, **quaranta** [conta: hostapi-metodi] oggi; operazioni strutturali dentro, `storage_*` fuori | Dopo il freeze, una capacità mancante è una feature che non potrà mai essere un plugin: ogni voce è decisa a verbale, comprese quelle escluse. Dentro: creare (che **rifiuta** un path occupato), rinominare (quella del kernel, che riscrive i backlink), il giro del cestino, `run_command`. Fuori con ragione: allegati (§14.1 — il modello ora c'è con la [0046](decisions/0046-l-anagrafe-del-vault.md), la capacità sarà additiva), cartelle (§14.3) e tempo differito, la cui ragione è stata **rifatta** — la premessa («il kernel non possiede thread») è caduta con la [0032](decisions/0032-il-runner-dei-job.md) e a reggere la conclusione è l'altra regola, per cui una sveglia informa ed è un evento ([0069](decisions/0069-cosa-sa-dire-un-abbonamento.md)). La **rete** era fuori con due bloccanti nominati, §9.1 e §7.3, caduti tutti e due ([0027](decisions/0027-il-lavoro-lungo-vede-il-vault.md), [0021](decisions/0021-il-confine.md)) senza che nessuno se ne accorgesse per settantasei verbali: **adesso è dentro** ([0097](decisions/0097-un-recinto-che-vale-anche-quando-nessuno-guarda.md)), ed è l'unica capacità il cui **permesso porta un parametro che si onora** — `fub:network` con la sua allowlist di host e `notify`/`progress`/`log`, che informano senza aspettare risposta — cioè eventi. `report_progress` sembra l'eccezione e non lo è: è la **porta** di un evento ([0035](decisions/0035-il-lavoro-lungo-si-racconta.md)) e non riapre la regola. La distanza fra ventidue e quaranta è tutta di **aggiunte**, cioè di minor — l'elenco si è chiuso a ciò che si toglie, non a ciò che cresce. |
...
| «Perdite silenziose non esistono per contratto» vale su un canale solo | **mitigato** | l'invariante era vera della sola coda eventi. Adesso vale su quattro canali: l'alimentazione degli indici **nomina** ciò che non ha preso ([0051](decisions/0051-l-alimentazione-risponde.md)), e l'esito di un handler, il flush e il panico di un provider diventano un `Event::Trouble` che il centro notifiche mostra ([0052](decisions/0052-cio-che-va-storto-e-un-evento.md)). La shell è stata convertita per intera ([0080](decisions/0080-un-guasto-si-dice-a-chi-sta-lavorando.md), §20.4): i suoi **14** `console.warn/error` chiamano il centro notifiche, e il salvataggio — che non aveva nemmeno un esito da mandarci — ne ha uno con quattro stati nella barra. E i **ventisette** `eprintln!` del backend che questa riga contava come non convertiti li ha smistati la [0062](decisions/0062-il-log-e-il-pavimento-l-evento-e-la-porta.md), col criterio *il log è il pavimento, l'evento è la porta*: apre la porta solo ciò che racconta una **perdita**, le diagnosi per chi sviluppa restano nel pavimento, e nei `crates/*/src` di produzione lo `stderr` è sceso a **zero**. Il caso di troncamento scoperto misurando la 0052 — il budget del dispatch, che svuotava la coda senza guardare cosa ci fosse dentro — lo ha chiuso la [0111](decisions/0111-il-budget-e-un-tetto-sul-lavoro.md): il budget è un tetto sul lavoro, non sui fatti |
...
| L'ecosistema di terzi è tutto dentro M5 | **dichiarato** | oggi un blocco di terzi è un **disegno**: `Trust::allows_active_content()` è vero solo per `Core` e una parte dichiarativa non riceve azioni ([0017](decisions/0017-chi-disegna-cio-che-il-core-non-conosce.md)). Il widget vero è l'iframe sandboxato, che quella stessa decisione manda a M5 con l'asset story e la CSP dedicata — quindi se M5 slitta, ciò che slitta con lui non è una feature ma la **classe** di ciò che un terzo può disegnare, e il piano oggi non ha un ripiego scritto. Accanto ci sta il gemello sull'editing, che adesso è **deciso** e in un altro modo: la [0104](decisions/0104-la-superficie-di-scrittura-si-presta.md) ha stabilito che la superficie di scrittura **si presta** — *«l'editor è della shell»* vuol dire questo editor, non l'editing — quindi un terzo che porta la propria esperienza di scrittura è un cliente previsto, non un rinvio a M5. Ciò che manca sono **due porte additive** (un evento di tastiera nel contratto; una via di disegno che non sia riservata a `Trust::Core`), e sta scritto come **buco dichiarato** accanto al metro in [plugin-boundary.md](architecture/plugin-boundary.md#cosa-non-può-essere-solo-un-guest-e-il-metro-per-deciderlo) |
| Un derivato per-path si scollega se la nota si rinomina ad app chiusa | **risolto** | il path è la chiave per sempre ([0043](decisions/0043-il-path-e-la-chiave.md)) e la migrazione segue le rinomine che il watcher vede; le altre lasciavano indietro spazio per-documento, bozze e versioni. Il prezzo era dichiarato in tre verbali e in nessun totale, ed è la strada che nessuno aveva guardato ad averlo chiuso (§23.1, [0099](decisions/0099-una-rinomina-che-non-ha-visto-nessuno.md)): riaccoppiare **per contenuto**, cioè l'apertura che riconosce un sparito e un comparso con la stessa impronta — uno a uno o niente, e nel dubbio non si accoppia e non si raccoglie |
| Il testo che l'utente sta scrivendo esce senza un permesso | **risolto** | `Selection.text` viaggiava sotto `Capability::Env` — «che ore sono e cosa guarda l'utente» — la cui `permission()` era `None`, cioè non dichiarabile in un manifest e non negabile da nessuna politica. La [0095](decisions/0095-cosa-guardo-e-cosa-sto-scrivendo.md) ha spaccato quella famiglia in **tre** (§23.5): l'orologio resta senza permesso perché è della macchina, *quale nota è aperta* è `fub:read-session`, *il testo selezionato* è `fub:read-selection`. L'ordine che la voce dichiarava è stato onorato: la rete della riga qui sopra è entrata dopo, con la [0097](decisions/0097-un-recinto-che-vale-anche-quando-nessuno-guarda.md) |
| Il freeze arriva prima delle firme che FEATURES richiede | presidiato | è ciò che le voci **P0** di [todo.md](todo.md) esistono per chiudere: è P0 tutto ciò che ha una forma di **contratto**, in qualunque capitolo. Con la [0051](decisions/0051-l-alimentazione-risponde.md) non ne restava nessuna aperta — e il rischio si era spostato dove è sempre stato davvero: non nel chiudere quelle trovate, ma nel **trovare quelle che nessun giro ha ancora visto**. Una c'era, e l'ha trovata una rilettura dei primi dieci verbali invece di un giro: la [§23.4](roadmap/23-cosa-costano-le-decisioni-chiuse.md#234-selection-ne-porta-una-sola-e-il-tipo-di-un-campo-non-è-additivo) — `ViewContext.selection` era singolare, e il multi-cursore non era un campo in più ma il **tipo** di quello che c'era. L'ha chiusa la [0093](decisions/0093-le-selezioni-sono-n-e-il-buffer-e-uno.md) (`selections: option<selection-set>`), e per undici giri nessuno ne ha più trovata una. La dodicesima l'ha trovata, ed è il fatto che questa riga esiste per registrare: le voci vive di [todo.md](todo.md) oggi sono **otto**, tutte della [seduta 26](roadmap/26-otto-gesti-che-nessuno-puo-dichiarare.md) — una P0, cinque P1, due P2, e sette delle otto di strato **contratto**. La P0 è la [§26.6](roadmap/26-otto-gesti-che-nessuno-puo-dichiarare.md#266-gli-appunti-sono-una-spunta-sola-e-le-domande-sono-due), e come la [§25.1](roadmap/25-sette-scelte-che-il-codice-ha-preso-senza-dirlo.md#251-una-rinomina-che-atterra-su-una-nota-viva) prima di lei **non è una firma** — ma per una ragione diversa, e la differenza è ciò che va letto qui. La §25.1 (chiusa dalla [0135](decisions/0135-una-rinomina-che-atterra-su-una-nota-viva.md)) era un dato che non si ricostruisce da niente: non scadeva col freeze, e stava in cima per il danno. La §26.6 è un **permesso**: `clipboard` è una spunta sola per leggere e per scrivere, e le due domande non sono la stessa domanda. Un permesso si può aggiungere anche dopo il freeze — è additivo — ma **spaccarne uno già dichiarato no**, perché il manifest che lo nomina è già stato scritto da qualcun altro. La sua finestra quindi non si chiude col freeze: si chiude **prima**, col primo manifest di terzi che chiede `clipboard`. È la scadenza più stretta che questo elenco abbia mai avuto, e non è il freeze a fissarla. Vale come misura del rischio più che come voce, e nello stesso verso di prima: anche questa P0 non stava in un buco, stava in una riga che il contratto aveva **dichiarato** — a nominarla non è servito un giro nuovo ma un elenco più fine. Le decisioni con una domanda aperta sono nella checklist di [M4](milestones/M4-wit-hardening.md); il dogfooding resta lo strumento che le scopre |
