...
| Quale ricerca ([0025](decisions/0025-la-ricerca-predefinita.md)) | **Di classe *omnisearch*, built-in, accesa di default** | Due motori sullo stesso vault sono due ranking e due risposte alla stessa domanda. La ricerca non è un pannello: è la strada verso quick switcher, palette, `RunSearch`, collezioni, `vault.replace`, e se il comportamento buono stesse in un plugin ognuna di quelle superfici dovrebbe sapere se quel plugin c'è. Conseguenza: `TextMode` non sa dire «a meno di un refuso», e `DocumentMatch.highlights` sono span dentro `snippet` — quindi ricerca nella nota aperta e salto all'occorrenza sono *inesprimibili*. Tre **P0** nella [seduta 21](roadmap/21-la-ricerca-predefinita.md), prima del freeze M4. |
...
| «Perdite silenziose non esistono per contratto» vale su un canale solo | aperto | l'invariante è vera della coda eventi. Altrove la perdita è **indicibile** (`IndexProvider::on_document_*` ritorna `()`), **scartata** (`let _ = handler.handle(…)`) o **detta a nessuno** (14 `eprintln!`, 12 `console.warn`). Mitigazione: [todo.md](todo.md), seduta 20; la metà che scade col freeze è §20.1 |
...
| Il costo di una capacità non è la firma, è il numero di host | **mitigato** | [0021](decisions/0021-il-confine.md): il rifiuto è un wrapper generico (`Guard<H, P: Policy>`) e `HostApi` è la somma di dieci famiglie, così «sola lettura» è un tipo che non ha le scritture invece di un tipo che ne rifiuta dodici |
...
| Il freeze arriva prima delle firme che FEATURES richiede | presidiato | è ciò che le voci **P0** di [todo.md](todo.md) esistono per chiudere: è P0 tutto ciò che ha una forma di **contratto**, in qualunque capitolo. Le decisioni con una domanda aperta sono nella checklist di [M4](milestones/M4-wit-hardening.md); il dogfooding resta lo strumento che le scopre |
