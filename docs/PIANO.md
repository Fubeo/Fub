...
| Un derivato per-path si scollega se la nota si rinomina ad app chiusa | **aperto** | il path è la chiave per sempre ([0043](decisions/0043-il-path-e-la-chiave.md)) e la migrazione segue le rinomine che il watcher vede; le altre lasciano indietro spazio per-documento, bozze e versioni. Il prezzo era dichiarato in tre verbali e in nessun totale: è la [§23.1](roadmap/23-cosa-costano-le-decisioni-chiuse.md#231-una-rinomina-fatta-ad-app-chiusa-scollega-tutto-ciò-che-è-indicizzato-per-path), e la strada che nessuno aveva guardato è riaccoppiare **per contenuto** |
| Il testo che l'utente sta scrivendo esce senza un permesso | **aperto** | `Selection.text` viaggia sotto `Capability::Env` — «che ore sono e cosa guarda l'utente» — la cui `permission()` è `None`, cioè non è dichiarabile in un manifest e nessuna politica la può negare. Il recinto sui path di `read-vault` si aggira selezionando, e il contesto si ripubblica ogni 150 ms. Innocuo finché non c'è rete, e la rete è la riga qui sopra: è la [§23.5](roadmap/23-cosa-costano-le-decisioni-chiuse.md#235-il-testo-che-lutente-seleziona-viaggia-sotto-una-capacità-che-nessuno-può-negare), e va decisa **prima** della §23.3 |
| Il freeze arriva prima delle firme che FEATURES richiede | presidiato | è ciò che le voci **P0** di [todo.md](todo.md) esistono per chiudere: è P0 tutto ciò che ha una forma di **contratto**, in qualunque capitolo. Con la [0051](decisions/0051-l-alimentazione-risponde.md) non ne restava nessuna aperta — e il rischio si era spostato dove è sempre stato davvero: non nel chiudere quelle trovate, ma nel **trovare quelle che nessun giro ha ancora visto**. Una c'era, e l'ha trovata una rilettura dei primi dieci verbali invece di un giro: la [§23.4](roadmap/23-cosa-costano-le-decisioni-chiuse.md#234-selection-ne-porta-una-sola-e-il-tipo-di-un-campo-non-è-additivo) — `ViewContext.selection` è singolare, e il multi-cursore non è un campo in più ma il **tipo** di quello che c'è. Vale come misura del rischio più che come voce: la P0 non stava in un buco, stava in una riga che il verbale aveva **dichiarato**. Le decisioni con una domanda aperta sono nella checklist di [M4](milestones/M4-wit-hardening.md); il dogfooding resta lo strumento che le scopre |
