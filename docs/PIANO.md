...
| Modifiche esterne | **Il rilevamento è un fatto che si chiede**: `IndexQuery::VaultStatus` → `VaultStatus { watching, sync_failures, last_sync_error }`, con **una** bandiera che il kernel presta e che tiene chi guarda ([decisione 0030](decisions/0030-il-rilevamento-si-puo-chiedere.md)) | Il file watcher è l'**unico** meccanismo con cui FubMD sa che qualcun altro ha toccato il vault: `reindex` gira solo all'apertura, non c'è riconciliazione periodica, e niente confronta mai la cache col disco. Prima nessuno chiedeva se fosse vivo (`is_watching` rispondeva *per costruzione*, e un debouncer morto diceva `true` per sempre) e la sincronizzazione per-path scartava il proprio esito con un `let _ =`. Da qui la promessa, esplicita: la verità è il vault sul disco, e le risposte di FubMD ne sono un riflesso aggiornato **solo quando `watching` è vero**; dove non lo è — network share, cloud drive, vault sincronizzati da fuori, CLI, PWA, mobile — la promessa è più piccola e dichiarata: ciò che passa da FubMD è coerente, ciò che passa da fuori si vede alla riapertura. Passa dal canale dati e non da un comando IPC perché una feature comandi non ne ha. |
...
| Capacità dell'`HostApi` ([decisione 0013](decisions/0013-elenco-delle-capacita.md)) | **L'elenco è chiuso**: ventitré metodi, con le operazioni strutturali dentro e `storage_*` fuori | Dopo il freeze una capacità che manca è una feature che **non potrà mai essere un plugin**, quindi ogni voce è stata decisa a verbale — comprese quelle che non entrano, o «non ci avevamo pensato» diventerebbe indistinguibile da «è stata una scelta». Dentro: creare (che **rifiuta** un path occupato — è l'unica differenza con `write_document`, e senza di essa un template che sbaglia la data cancella una nota), rinominare (quella del kernel, che **riscrive i backlink**: non ce n'è una nuda, perché due semantiche sotto un nome sono una trappola), il giro del cestino, e `run_command`, che eredita modo, attore e lotto invece di prenderli come argomenti — una simulazione non diventa reale invocando qualcuno, e una macro di tre comandi resta una cosa sola. Fuori, con la ragione: allegati (§14.1 — che da allora **il modello ce l'ha**, con la [0046](decisions/0046-l-anagrafe-del-vault.md): un allegato si elenca, si sposta portandosi dietro i riferimenti e si vede mancare dal controllo di salute, quindi la capacità che scrive sarà additiva quando qualcuno la chiederà), rete (i suoi due bloccanti, §9.1 e §7.3, sono **caduti** con la [0027](decisions/0027-il-lavoro-lungo-vede-il-vault.md) e la [0021](decisions/0021-il-confine.md): resta che le allowlist dei permessi non sono ancora applicate), tempo differito (§8.3), cartelle (§14.3), e `notify`/`progress`/`log`, che informano senza aspettare risposta — cioè la definizione di un **evento**, non di una capacità. `storage_*` volatile è stato **tolto**: con `data_*` e le impostazioni non aveva più casi d'uso, e toglierlo dopo il freeze sarebbe stata una major. Il ventitreesimo è arrivato con la [0035](decisions/0035-il-lavoro-lungo-si-racconta.md) e non riapre la regola: `report_progress` è la **porta** di un evento (`job-progress`), come lo è `emit`, e c'è perché un job non conosce il proprio `JobId` — l'id non è un parametro, quindi nessuno può raccontare il progresso di un altro. |
