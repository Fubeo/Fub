...
#0001-versionref-attraversa-lipc
)
|
`VersionRef`
 attraversa l'IPC, e per lui 
`fub-app`
 dipende da 
`fub-features`
|
`crates/fub-app/Cargo.toml`
|
 2026-07-31 
|
|
...
|
`restore_from_trash`
 limita il ripristino ai soli documenti (no asset) 
|
...
|
 Inerzia del contesto di 
`Session`
 alla disattivazione di un plugin 
|
`crates/fub-kernel/src/workspace.rs`
|
 2026-07-31 
...
#0004-accumulo-di-sidecar-orfani-in-fubdatatrash-per-cancellazioni-esterne
)
|
 Accumulo di sidecar orfani in 
`.fub/data/trash/`
 per cancellazioni esterne 
|
...
|
 Permanenza del contatore 
`sync_failures`
 in 
`VaultStatus`
 fino alla chiusura 
|
`crates/fub-kernel/src/index/core.rs`
|
 2026-07-31 
|
|
...
#0007-hostclose_vault-e-with_session-falliscono-se-la-cartella-del-vault-viene-eliminata-o-spostata-su-disco
)
|
`Host::close_vault`
 e 
`with_session`
 falliscono se la cartella del vault viene eliminata o spostata su disco 
|
...
#0008-set_plugin_enabled-non-disattiva-i-plugin-con-job-in-volo-e-salta-plugin-deactivate
)
|
`set_plugin_enabled(false)`
 non disattiva i plugin con job in volo e salta 
`Plugin::deactivate`
|
`crates/fub-host/src/session.rs`
|
...
|
`Banco::con_spia`
 cancella gli eventi della semina e della scansione iniziale 
|
`crates/fub-testkit/src/lib.rs`
|
 2026-07-31 
|
|
...
|
 Notifiche di errore nei comandi Tauri 
`close_vault`
 e 
`set_plugin_enabled`
 ritornano stringhe non tipizzate 
|
...
#0011-omissione-di-tipi-fondamentali-di-traitsrs-dai-re-export-di-livello-radice-in-fub-abi
)
|
 Omissione di tipi fondamentali di 
`traits.rs`
 dai re-export di livello radice in 
`fub-abi`
|
`crates/fub-abi/src/lib.rs`
|
 2026-07-31 
|
|
...
|
 Divergenza potenziale fra i default di 
`OptionMap`
 e i booleani nelle capabilities 
|
`crates/fub-abi/src/options.rs`
|
 2026-07-31 
|
|
...
#0014-dettaglio-diagnostico-del-rifiuto-di-documentsourcebytes-in-formatprovider
)
|
 Dettaglio diagnostico del rifiuto di 
...
#0016-accumulo-di-listener-in-onlingua-senza-unlisten
)
|
 Accumulo di listener in 
`onLingua`
 senza meccanismo di rimozione 
|
...
#0017-list_trash-fallisce-per-intero-se-nel-cestino-trash-e-presente-un-symlink-rotto
)
|
`list_trash()`
 fallisce per intero se nel cestino 
`.trash/`
 è presente un symlink rotto 
|
`crates/fub-kernel/src/vault.rs`
|
 2026-07-31 
...
#0018-scansione-lineare-o-n--m-con-normalizzazione-unicode-nella-risoluzione-dei-link-rotti
)
|
 Scansione lineare $O(N \cdot M)$ con normalizzazione Unicode nella risoluzione dei link rotti 
|
`crates/fub-kernel/src/index/core.rs`
|
 2026-07-31 
...
`data_root`
 e 
`TRASH_DIR`
 compongono path relativi sensibili al cambio di working directory 
|
`crates/fub-kernel/src/vault.rs`
|
 2026-07-31 
...
#0020-disallineamento-sul-conteggio-e-lelenco-delle-famiglie-di-hostapi-in-traitsmd
)
|
 Disallineamento sul conteggio e l'elenco delle famiglie di 
`HostApi`
 in 
`traits.md`
|
`docs/architecture/traits.md`
|
 2026-07-31 
|
|
...
#0021-omissione-dei-moduli-i18n-e-state-localets-nella-mappa-dellelbero-frontend-in-shellmd
)
|
 Omissione dei moduli 
`i18n`
 e 
`state/locale.ts`
 nella mappa dell'albero frontend in 
`shell.md`
|
...
#0022-riferimento-di-riga-obsoleto-per-schema_version-in-versioningrs-allinterno-di-versionamentomd
)
|
 Riferimento di riga obsoleto per 
`SCHEMA_VERSION`
 in 
`versioning.rs`
 all'interno di 
...
#0023-omissione-della-specifica-di-troncamento-dellultima-estensione-per-docidpage_name-in-data-modelmd
)
|
 Omissione della specifica di troncamento dell'ultima estensione per 
`DocId::page_name()`
 in 
`data-model.md`
|
...
`click`
 pendente in 
`showContextMenu`
 dopo la chiusura del menu via tastiera 
|
`frontend/src/ui/menu.ts`
|
 2026-07-31 
|
|
...
#0025-riconciliazione-incompleta-di-select-radio-e-attributi-input-in-uinode
)
|
 Riconciliazione incompleta di 
`select`
, 
`radio`
 e attributi 
...
|
 Cambio di scheda nella sidebar forzato al termine di una ricerca in volo 
|
`frontend/src/panels/search.ts`
|
 2026-07-31 
|
|
...
|
 Inclusione forzata di 
`false`
 nei parametri booleani opzionali in 
`argsFromForm`
|
`frontend/src/ui/palette.ts`
|
 2026-07-31 
|
|
...
#0029-mancata-esposizione-di-editorviewdestroy-nel-wrapper-delleditor
)
|
 Mancata esposizione di 
`EditorView.destroy`
 nel wrapper dell'editor 
|
`frontend/src/editor/editor.ts`
|
 2026-07-31 
...
|
 Race condition durante il salvataggio asincrono con input continuo 
|
`frontend/src/panels/document.ts`
|
 2026-07-31 
...
#0031-sovrascrittura-dello-stato-chiuso-dellanteprima-con-rendering-stale
)
|
 Sovrascrittura dello stato chiuso dell'anteprima con rendering stale 
|
`frontend/src/panels/preview.ts`
|
 2026-07-31 
|
|
...
#0032-lock-contention-e-deadlock-potenziali-nellhost-risolto
)
|
 Lock contention e deadlock potenziali nell'Host (Risolto) 
|
`crates/fub-host/src/session.rs`
|
 2026-07-31 
...
|
 Race condition su chiamate concorrenti a 
`openDocument`
|
`frontend/src/panels/document.ts`
|
...
|
 Race condition da concorrenza non gestita in 
`refreshFromKernel`
 (out-of-order UI updates) 
|
`frontend/src/panels/explorer.ts`
|
 2026-07-31 
|
|
...
#0035-application-crash-on-poisoned-locks
)
|
 Application Crash on Poisoned Locks 
|
`crates/fub-app/src/lib.rs`
|
 2026-07-31 
...
|
 Silent Event Loss During Initialization 
|
`crates/fub-app/src/lib.rs`
|
 2026-07-31 
...
|
 Unhandled Serialization Failures in Event Bridge 
|
`crates/fub-app/src/lib.rs`
|
 2026-07-31 
...
#0038-synchronous-io-and-blocking-ipc-commands
)
|
 Synchronous I/O and Blocking IPC Commands 
|
`crates/fub-app/src/lib.rs`
|
 2026-07-31 
...
|
 TOCTOU Race Condition in propose_free_name 
|
`crates/fub-app/src/lib.rs`
|
 2026-07-31 
...
#0040-massive-synchronous-io-bottleneck-in-vault_replace
)
|
 Massive Synchronous I/O Bottleneck in vault_replace 
|
`crates/fub-features/src/commands.rs`
|
 2026-07-31 
|
|
...
#0041-deadlock-risk-in-searchindexcommit-double-checked-locking
)
|
 Deadlock Risk in SearchIndex::commit (Double-Checked Locking) 
|
...
#0042-inconsistent-state-management-in-searchindexup_to_date-causing-data-loss
)
|
 Inconsistent State Management in SearchIndex::up_to_date causing Data Loss 
|
`crates/fub-features/src/search.rs`
|
 2026-07-31 
...
|
 Out-of-Memory (OOM) Risk due to Unbounded Pagination in search 
|
`crates/fub-features/src/search.rs`
|
 2026-07-31 
...
|
 Silent failure on meta.json parsing allows directory hijacking 
|
...
|
 Massive memory bottleneck during index rebuild 
|
`crates/fub-features/src/versioning.rs`
|
 2026-07-31 
...
#0046-holding-mutex-locks-across-io-boundaries
)
|
 Holding Mutex locks across I/O boundaries 
|
`crates/fub-features/src/versioning.rs`
|
 2026-07-31 
...
|
 Action Reveal lacks doc_id in payload, risking cross-document jumps 
|
`crates/fub-features/src/outline.rs`
|
 2026-07-31 
|
|
...
|
 Incomplete attribute escaping in escape_attr 
|
`crates/fub-features/src/blocks.rs`
|
 2026-07-31 
|
|
...
|
 Severe Redundant Full-Document Reads on Cursor Movement 
|
`crates/fub-features/src/stats.rs`
|
 2026-07-31 
|
|
...
|
 Double Text Traversal for Statistics 
|
`crates/fub-features/src/stats.rs`
|
 2026-07-31 
...
|
 Continuous String Allocation in Render Loop 
|
`crates/fub-features/src/tags.rs`
|
 2026-07-31 
|
|
...
|
 Hardcoded UI Element Keys Risking State Leakage 
|
`crates/fub-features/src/tags.rs`
|
 2026-07-31 
...
|
 Perdita di blocchi HTML crudi durante la serializzazione 
|
...
#0054-perdita-del-testo-di-fallback-per-gli-inlinecustom-sconosciuti
)
|
 Perdita del testo di fallback per gli Inline::Custom sconosciuti 
|
`crates/fub-format-markdown/src/serialize.rs`
|
 2026-07-31 
...
|
 Rottura dei Code Block contenenti backticks 
|
`crates/fub-format-markdown/src/serialize.rs`
|
 2026-07-31 
...
|
 Esportazione documento singolo con Frontmatter corrotto 
|
`crates/fub-format-markdown/src/transfer.rs`
|
 2026-07-31 
|
|
...
|
 Assenza di contesto (context) per i Link presenti in Intestazioni e Tabelle 
|
...
#0058-restore_from_trash-legge-il-sorgente-dal-cestino-ma-scrive-a-target-senza-delete-atomico
)
|
`restore_from_trash`
 scrive il documento prima di cancellare la copia nel cestino: due copie in caso di crash 
|
`crates/fub-kernel/src/workspace.rs`
|
 2026-07-31 
...
#0059-link_rewrite_plan-verifica-lambiguità-su-metas-ma-non-su-entries-per-i-wikilink
)
|
`link_rewrite_plan`
 verifica l'ambiguità su 
`metas`
 ma non su 
`entries`
 (allegati omonimi sfuggono) 
|
`crates/fub-kernel/src/workspace.rs`
|
 2026-07-31 
...
#0060-section_of-usa-usize-max-come-sentinella-di-fine-sezione-overflow-teorico-su-file-enormi
)
|
`section_of`
 usa 
`usize::MAX`
 come sentinella di fine sezione: overflow teorico su span di documenti enormi 
|
`crates/fub-kernel/src/workspace.rs`
|
 2026-07-31 
...
#0061-insert_sorted-e-remove_sorted-usano-binary_search_by-su-un-comparatore-non-strettamente-totale
)
|
`insert_sorted`
/
`remove_sorted`
 usano 
`binary_search_by`
 con un comparatore che non è strettamente totale su alias duplicati 
|
...
#0062-duplicate-links-keep-multiplicity-i-backlink-duplicati-non-vengono-deduplicati-nel-set-refs_by_key
)
|
 Backlink duplicati non vengono deduplicati in 
`refs_by_key`
: 
`unregister_links`
 può lasciare riferimenti orfani 
|
`crates/fub-kernel/src/graph.rs`
|
 2026-07-31 
...
#0063-jobbell-usa-un-contatore-u64-che-non-si-azzera-mai-overflow-teorico-dopo-lunghissimo-uptime
)
|
`JobBell`
 usa un contatore 
`u64`
 che non si azzera mai: overflow teorico dopo lunghissimo uptime 
|
...
#0064-subscription-recv-con-canale-disconnected-restituisce-overflow-e-poi-err-invece-di-segnalare-la-disconnessione
)
|
`Subscription::recv`
 con canale 
`Disconnected`
 restituisce un 
`Overflow`
 sintetico ma poi chiama 
`recv`
 bloccante su un canale già chiuso 
|
`crates/fub-kernel/src/bus.rs`
|
 2026-07-31 
|
|
...
#0065-notifywatcher-tiene-il-lock-esclusivo-del-workspace-durante-lintera-raffica-debounced-e-il-flush-su-disco
)
|
`NotifyWatcher`
: lock esclusivo del workspace tenuto per tutto il ciclo di eventi + 
`flush_indexes`
 (I/O su disco sotto lock) 
|
`crates/fub-host/src/watcher.rs`
|
 2026-07-31 
|
|
...
#0066-nel-ramo-err-del-watcher-lerrore-trouble-non-viene-mai-emesso-se-il-lock-del-workspace-e-avvelenato
)
|
 Nel ramo 
`Err`
 del watcher il flag 
`watching`
 viene abbassato prima del lock: se il lock è avvelenato l'evento 
`Trouble`
...
#0067-bundleregistrystop-non-verifica-linvariante-di-unicità-dellarc-prima-di-chiamare-deactivate
)
|
`BundleRegistry::stop`
: 
`Arc::get_mut`
 fallisce silenziosamente se un job è ancora in volo, 
`Plugin::deactivate`
 non viene chiamato e nessun assert codifica l'invariante 
|
...
#0068-checkpath-namingnew-accetta-nomi-con-spazi-in-testa-che-normalized-trasforma-in-file-nascosti
)
|
`check(path, Naming::New)`
 accetta nomi con spazi in testa che 
`normalized`
 trasforma in file nascosti 
|
...
#0069-in-caso-di-panico-in-workspacebatch-batch-resta-attivo-e-blocca-per-sempre-il-dispatch-degli-eventi
)
|
 In caso di panico in 
`workspace.batch()`
, 
`batch`
 resta attivo e blocca per sempre il dispatch eventi 
|
`crates/fub-kernel/src/workspace.rs`
|
 2026-07-31 
...
#0070-prefix_len_ci-in-occurrencesrs-confronta-caratteri-unicode-char-by-char-fallendo-su-espansioni
)
|
`prefix_len_ci`
 in 
`occurrences.rs`
 confronta caratteri Unicode char-by-char fallendo su espansioni 
|
...
#0071-undostackpush-usa-vecremove0-con-spostamenti-on-ad-ogni-inserimento-oltre-il-tetto
)
|
`UndoStack::push`
 usa 
`Vec::remove(0)`
 con spostamenti $O(N)$ ad ogni inserimento oltre il tetto 
|
`crates/fub-kernel/src/undo.rs`
|
 2026-07-31 
|
|
...
#0072-accumulo-di-entry-orfane-in-flagslive-per-cancel_job-con-jobid-inesistenti
)
|
 Accumulo di entry orfane in 
`Flags::live`
 per 
`cancel_job`
 con 
...
#0073-scrittura-sincrona-su-disco-in-set_view_state-blocca-il-thread-ipc-di-tauri
)
|
 Scrittura sincrona su disco in 
`set_view_state`
 blocca il thread IPC di Tauri 
|
`crates/fub-app/src/lib.rs`
|
 2026-07-31 
...
|
 Mancato aggiornamento del timestamp 
`last_opened`
 alla riapertura di un vault già aperto 
|
`crates/fub-host/src/session.rs`
|
 2026-07-31 
...
|
 Impossibilità di ripristinare il nome di default del vault tramite 
`set_vault_look`
|
...
#0076-panico-in-vaultsessionclose-con-lock-del-workspace-avvelenato
)
|
 Panico in 
`VaultSession::close`
 con lock del workspace avvelenato e perdita di dati 
|
`crates/fub-host/src/session.rs`
|
 2026-07-31 
...
#0077-fallimento-di-fallback-per-config_dir-se-fub-config-non-è-scrivibile
)
|
 Fallimento di fallback per 
`config_dir`
 se la cartella 
`fub-config`
 non è skrivibile 
|
`crates/fub-host/src/config.rs`
|
 2026-07-31 
|
|
...
#0078-silenziosa-terminazione-del-thread-bridgers-su-disconnessione-del-bus
)
|
 Silenziosa terminazione del thread 
`bridge.rs`
 su disconnessione del bus senza log 
|
`crates/fub-host/src/bridge.rs`
|
 2026-07-31 
|
|
...
#0079-omissione-dellattributo-data-embed-block-per-la-transclusione-dei-blocchi-nei-wikilink-incorporati
)
|
`render_html`
 omette l'attributo 
`data-embed-block`
 nei wikilink incorporati 
`![[Nota#^blocco]]`
|
...
#0080-serializzazione-errata-di-linktargetwiki-con-ancora-di-blocco-senza-prefisso-
)
|
`serialize`
 genera wikilink con ancora di blocco senza 
`#`
 (es. 
`[[page^block]]`
 anziché 
...
|
 Disallineamento dello span nei link incorporati (
`![[...]]`
) fra parser AST comrak e fallback testuale 
|
`crates/fub-format-markdown/src/parse.rs`
|
 2026-07-31 
...
`[ ]`
 (
`Some(' ')`
) come completato e non spunta mai i task da fare 
|
`crates/fub-features/src/commands.rs`
|
 2026-07-31 
...
|
 Memory leak e listener pendenti in 
`pickIcon`
 alla riapertura del selettore 
|
`frontend/src/ui/menu.ts`
|
 2026-07-31 
...
|
 Memory leak di listener 
`keydown`
 su 
`document`
 alla riapertura del grafo 
|
...
#0085-conflitto-globale-sullattributo-name-nei-campi-radio-tra-form-differenti
)
|
 Conflitto globale sull'attributo 
`name`
 nei campi 
`radio`
 tra form differenti 
...
|
 Mancata gestione degli errori nelle azioni asincrone delle viste dichiarative 
|
...
#0087-race-condition-nel-fallback-da-patch-a-renderdeclaredview
)
|
 Race condition nel fallback da 
`patch`
 a 
`renderDeclaredView`
...
#0088-inconsistenza-dello-stato-delle-viste-alla-chiusuraapertura-se-listviews-fallisce
)
|
 Inconsistenza dello stato delle viste se 
`api.listViews`
 fallisce 
|
`frontend/src/ui/views.ts`
...
| [0032](#0032-interruzione-della-potatura-in-forget_vault-al-primo-errore-io) | Interruzione della potatura in `forget_vault` al primo errore I/O | `crates/fub-host/src/session.rs` | 2026-07-31 |
| [0033](#0033-divergenza-in-memoria-su-fallimento-di-set_setting-in-set_plugin_enabled) | Divergenza in memoria su fallimento di `set_setting` in `set_plugin_enabled` | `crates/fub-host/src/session.rs` | 2026-07-31 |
| [0034](#0034-cambio-di-vault-corrente-alfabetico-alla-chiusura) | Cambio di vault corrente alfabetico alla chiusura | `crates/fub-host/src/session.rs` | 2026-07-31 |
| [0035](#0035-eventi-pendenti-non-drenati-in-deactivate_plugin-senza-indici) | Eventi pendenti non drenati in `deactivate_plugin` se il plugin non ha indici | `crates/fub-kernel/src/workspace.rs` | 2026-07-31 |
---
## 0032 — Interruzione della potatura in `forget_vault` al primo errore I/O
...
---
## 0033 — Divergenza in memoria su fallimento di `set_setting` in `set_plugin_enabled`
...
---
## 0034 — Cambio di vault corrente alfabetico alla chiusura
