...
graph TD
    %% Legenda stili
    classDef abi fill:#4a154b,stroke:#e01e5a,stroke-width:2px,color:#fff;
    classDef kernel fill:#1d1c1d,stroke:#616061,stroke-width:2px,color:#fff;
    classDef host fill:#0a4a3a,stroke:#2eb67d,stroke-width:2px,color:#fff;
    classDef wasm fill:#1a3a5a,stroke:#36c5f0,stroke-width:2px,color:#fff;
    classDef providers fill:#1a2a4a,stroke:#4a6fa5,stroke-width:2px,color:#fff;
    classDef app fill:#4a2a0a,stroke:#ecb22e,stroke-width:2px,color:#fff;
    classDef testkit fill:#3a1a3a,stroke:#a040a0,stroke-width:2px,color:#fff;

    ABI["📜 fub-abi<br><i>types + traits + wit</i>"]:::abi
    KERNEL["⚙️ fub-kernel<br><i>pure logic, 0 IO deps</i>"]:::kernel
    SDK["🧰 fub-sdk<br><i>helpers for providers</i>"]:::providers
    MD["📝 fub-format-markdown<br><i>comrak format provider</i>"]:::providers
    FEAT["✨ fub-features<br><i>10 official features</i>"]:::providers
    HOST["🏰 fub-host<br><i>mounter + runtime + watcher</i>"]:::host
    WASM["🔌 fub-wasm-host<br><i>wasmtime component model</i>"]:::wasm
    APP["🖥️ fub-app<br><i>tauri glue + window</i>"]:::app
    TESTKIT["🧪 fub-testkit<br><i>test harness (dev-only)</i>"]:::testkit

    %% Dipendenze di produzione (solide)
    KERNEL --> ABI
    SDK --> ABI
    MD --> ABI
    MD --> SDK
    ...
    %% Dipendenze di test/dev (tratteggiate)
    TESTKIT -.-> ABI
    TESTKIT -.-> KERNEL
    TESTKIT -.-> SDK
    FEAT -.-> KERNEL
...
> 🟣 viola: contratto (`fub-abi`) | ⚫ grigio scuro: core (`fub-kernel`) | 🔵 blu: provider (`fub-format-markdown`, `fub-features`, `fub-sdk`) | 🟢 verde scuro: montaggio (`fub-host`) | 🔷 azzurro: runtime WASM (`fub-wasm-host`) | 🟠 arancio: applicazione (`fub-app`) | 🟪 magenta: test harness (`fub-testkit`, solo `[dev-dependencies]`)

### Le quattro regole del grafo

Le frecce non sono arbitrarie: rispondono a quattro regole precise.

| Regola | Perché |
|---|---|
| **1. `fub-abi` sta in cima, non dipende da niente nel workspace, e non conosce né comrak né wasmtime né tauri** | È il contratto. Se dipendesse da una libreria pesante, qualunque cosa lo importi — compresi i plugin terzi — se la porterebbe dietro. |
| **2. `fub-kernel` non conosce I/O pesante** | Zero dipendenze da comrak, tauri, wasmtime, reqwest. Tutta la logica di indicizzazione, grafi, risoluzione link e dispatch è codice puro, testabile in RAM senza toccare il disco. |
| **3. `fub-host` non conosce tauri** | È il composition root del backend. Monta il kernel, avvia i thread, gestisce il watcher. Deve potersi avviare da un test, da una CLI, da un server, senza una webview. |
| **4. `fub-wasm-host` è l'unico posto dove vive wasmtime** | Wasmtime aggiunge ~30MB al binario e minuti di compilazione. Isolarlo in un crate a parte permette di compilare e testare tutto il resto senza toccarlo. |

### La superficie di scrittura

C'è un'altra regola che il disegno mostra: **un plugin non può toccare la
struttura di un altro plugin.**

```mermaid
graph LR
    P1["Plugin A"] -->|HostApi| GUARD["Guardia permessi"]
    P2["Plugin B"] -->|HostApi| GUARD
    GUARD -->|consentito| KERNEL["fub-kernel"]
    GUARD -->|negato| ERR["PermissionDenied"]
```

Un plugin chiama `HostApi`. La guardia controlla i permessi dichiarati nel
manifest del plugin prima di inoltrare la chiamata al kernel. Se il permesso non
c'è, la chiamata si ferma:
...
L'elenco a indentazione in [02-componenti/01-panoramica.md](../02-componenti/01-panoramica.md) non è un
...
flowchart TB
    ...
    subgraph PROC ["🪟 un processo — fub-app, Tauri v2"]
        ...
        subgraph S1 ["VaultSession — uno per vault aperto"]
            direction TB
            WSL["Custody&lt;Workspace&gt; — un Arc e un RwLock<br>chi legge condivide, chi chiama un provider no"]:::core
...
| Webview | Uno. Il core lo considera **privilegiato**: per questo `UiNode::Html` è negato al codice non fidato. | [07-ui/02-il-protocollo-ui-node.md](../07-ui/02-il-protocollo-ui-node.md) |
...
metterlo dietro un lucchetto è chi monta, dentro `Custody`
([custody.rs:122](../../crates/fub-host/src/custody.rs)). La `Custody` ha
...
Il conto torna con [07-ui/02-il-protocollo-ui-node.md](../07-ui/02-il-protocollo-ui-node.md), che oggi dice le stesse
