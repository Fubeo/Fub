...
- [ ] **L'escape hatch del modello esiste, il suo disegno no.** Il rendering di
      ...
      (`format-markdown/src/render.rs:103-140`); ogni altro kind cade in un ramo
      generico che produce `<div class="block-{kind}">` col contenuto dentro
      (`:125-140`). Il primo giro su questa voce diceva «non produce nulla, in
...
- [ ] **Due booleani** (`parse_tags`, `parse_wikilinks`, `format.rs:42-46`) contro
...
- [ ] **`PluginPermissions` sono tre booleani** (`abi/traits.rs:996-1001`) contro
