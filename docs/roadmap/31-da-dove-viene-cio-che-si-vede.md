...
[0167](../decisions/0167-un-colore-ha-una-ricetta.md); la casella dei due
gradini (`--bg-panel`, `--bg-active`) l'ha chiusa la
[0170](../decisions/0170-una-componente-un-anatomia.md) (§31.4)*
...
*pelle · shell · **P1** — **chiusa** dalla
[0170](../decisions/0170-una-componente-un-anatomia.md)*
...
- [x] **L'inventario si chiude prima di cominciare**: l'elenco dei componenti si
      ...
      evitato che una misura diventasse un cantiere. *Fatta: trentaquattro
      componenti in `frontend/src/theme/serie/anatomia.ts`
      (`grep -c '^  { name:' …`); un componente nuovo è una voce nuova.*
- [x] **Ogni componente dichiara i suoi stati** — riposo, hover, premuto,
      ...
      *Fatta: sette stati (`rest`, `hover`, `pressed`, `selected`, `focused`,
      `disabled`, `dragging`) e centoottantotto hook nella stessa tabella;
      `catalog.ts` legge `COMPONENTS`. Presidio bidirezionale:
      `npm test -- --run src/theme/serie/anatomia.test.ts`.*
...
- [x] **La pelle si scrive a pezzi e si monta in un file**: il caricatore vuole
      ...
      fossero.*
- [x] **Le icone hanno un tratto solo**: ventuno disegnate a mano sono coerenti
      ...
      dichiarati. *Fatta: ventuno nomi, griglia 24, dimensione 16, tratto 1.6,
      `currentColor`, cap/join tondi; il renderer usa le costanti e il gear del
      pannello passa da `icon('settings')`. Presidio:
      `npm test -- --run src/ui/icons.test.ts src/graph/physics-panel.test.ts`.*
- [x] **Il suggerimento è un componente della shell**, non un attributo del
      ...
      come `intrappolaFuoco`, e la sua pelle è del tema. *Fatta: `ui/tooltip.ts`
      con ritardo, ARIA e tasto accanto; zero `title` nativi
      (`npx vitest run src/ui/native-title.test.ts`) e quattro indicatori di
      tasto.*
...
3. ~~**La §31.4.**~~ Fatta: anatomia, vocabolario degli hook, icone, tooltip
