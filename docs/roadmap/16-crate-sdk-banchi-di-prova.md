...
- [ ] **18 helper `vault()`/`workspace()`** negli integration test e **14
      `impl FormatProvider` giocattolo**, di cui tre chiamati letteralmente
      `PlainProvider` in tre file diversi (`trash.rs:30`, `invoke_command.rs:34`,
      `structural_host.rs:35`). Ogni test che tocca il kernel si costruisce da
      capo vault temporaneo, registry, provider minimo e asserzioni sugli eventi.
...
*ex §4.7 · presidi · **P1** — dopo la 16.2*
...
*ex §4.1 · presidi · **P1** — la fixture non scala a trenta tipi nuovi*
