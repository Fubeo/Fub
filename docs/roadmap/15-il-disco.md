...
- [x] **Classe da Percorso:** La struttura del percorso determina la classe del
      ...
      parametro errato.
- [ ] **Implementazione su HostApi:** Le due capacità aggiuntive `cache_read` e
      `cache_write` entreranno in `HostApi`. Includono le compagne di
      `data_list` e `data_remove`. Lo spazio autorevole dei plugin sale da
      `.fub/data/plugins/<id>/` a `.fub/plugins/<id>/`. Segue la regola di
      rename della 0048. Implementabile dopo M3. Entrerà in uso prima di M5.
      Riguarda gli otto inquilini (plugin). I due plugin che subiscono
      variazioni (versioning e ricerca) si adegueranno.
