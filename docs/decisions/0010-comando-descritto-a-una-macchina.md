...
faccia su *più note insieme* o sulle impostazioni. La [decisione 0009](../decisions/0009-registro-dei-comandi.md) gli dà il registro;
...
- [x] **`CommandSpec` descrive gli argomenti**: `{ id, title, description,
      ...
      schema a sé e **non** i nodi del §2.1: dichiarare *cosa serve* e disegnare
...
- [x] **La simulazione è un modo di invocare**: `invoke(…, InvokeMode::DryRun)`
      ...
      `EditRequest` per documento ([decisione 0008](../decisions/0008-modifica-chirurgica.md)). E non è una cortesia di chi
...
- [ ] **Le impostazioni scrivibili da un programma sono un sottoinsieme
      dichiarato**: lo schema del §11.1 deve dire quali chiavi sono modificabili
...
- [ ] **L'attribuzione va nel lotto, non nel log dell'app**: chi ha chiesto
      l'operazione (utente, comando, modello, prompt) è la [decisione 0012](../decisions/0012-origine-degli-eventi.md) (origine degli
      eventi) applicata alla [decisione 0011](../decisions/0011-il-lotto.md) (il lotto). L'audit trail di 22.4 è quel campo
      più il journal del §15.2; senza il campo, «cosa ha cambiato l'AI ieri» si
      può solo indovinare dai timestamp.
      *Metà fatta ([decisione 0011](../decisions/0011-il-lotto.md) + [decisione 0012](../decisions/0012-origine-degli-eventi.md)), e la spunta resta giù apposta.* Fatto: chi
      ...
      chiede che qualcuno l'abbia scritta da qualche parte — il journal del §15.2.
      ...
      nominava prima che la [decisione 0012](../decisions/0012-origine-degli-eventi.md) esistesse.
...
**Fatto insieme alla [decisione 0009](../decisions/0009-registro-dei-comandi.md), con quattro decisioni e due voci ancora aperte.**
...
*Il consenso non è una capacità dell'`HostApi`.* Questa voce lo dava per scontato
(«è una capacità dell'`HostApi`, [decisione 0013](../decisions/0013-elenco-delle-capacita.md): la conferma»), e questo giro dice di no,
...
chiamante a simulare prima. L'obbligo, se sarà, è una policy del §7.3 sopra
...
l'approvazione, applicarlo fallisce con `Conflict` ([decisione 0008](../decisions/0008-modifica-chirurgica.md)) invece di
sovrascrivere — l'anteprima non è un'ipotesi vaga, è una promessa verificabile.

*Resta fuori, dichiarato:* le **impostazioni scrivibili da un programma** (§11.1:
c'è il vocabolario, non lo schema); l'**attribuzione** ([decisione 0012](../decisions/0012-origine-degli-eventi.md) + [decisione 0011](../decisions/0011-il-lotto.md): un campo
...
parziale** e l'**interruzione a metà** (22.4), che chiedono il lotto ([decisione 0011](../decisions/0011-il-lotto.md)).
