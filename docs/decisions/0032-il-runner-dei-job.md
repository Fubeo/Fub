...
- **La coda non la drenava nessuno.** `spawn_job` accodava, `complete_job`
  ...
  al pool il `JobHost` da passargli, la [0031](0031-chi-possiede-i-bundle.md)
  gli aveva dato **a chi chiedere il corpo**. Mancava chi girasse la chiave.
...
- **Un provider che pania si portava via il vault.** `view_action` e
  ...
  fa passare il parse del formato e l'alimentazione degli indici: da lì un
  panico avvelenava il `RwLock`, e i `.write().unwrap()` di chi monta lo
  traducevano in un panico su **ogni** comando successivo — il vault
  irraggiungibile fino al riavvio. La
  [0024](0024-chi-legge-non-aspetta-chi-legge.md) ne aveva tolto una metà (chi
  **disegna** gira sotto prestito condiviso, e un prestito condiviso non si
  avvelena); restava quella di chi **agisce**, che non è la metà meno probabile
  — con un'estensione installata, un handler di comando che pania è il caso
  normale.
...
- **N thread dedicati, non un worker solo e non un drenaggio sincrono.** Un
  worker solo vorrebbe dire che un job che aspetta la rete tiene fermo un job
  che calcola; un drenaggio sincrono — «ogni tanto, dentro un comando» —
  vorrebbe dire che il lavoro lungo gira nel giro che esiste per non farcelo
  girare.
...
- **Il campanello conta, non accende.** `queued` è *quanti job sono stati
  accodati da sempre*: chi drena legge il conto **prima** di drenare e poi
  aspetta che cambi. Con un booleano, un job accodato nella finestra fra il
  drenaggio e l'attesa resterebbe fermo fino al successivo — il bug che si vede
  una volta al mese e non si riproduce mai.

### La cancellazione

- **Non aggiunge nessuna capacità, ne toglie.** Nel contratto non compare
  niente: nessun `is_cancelled()` che un job debba ricordarsi di chiamare,
  nessun token da passare in giro. C'è una bandiera che il runner alza, e da
  quel momento l'host del job **rifiuta** — `PluginError::Cancelled` alla
  capacità successiva. Un job scritto prima che la cancellazione esistesse si
  ferma comunque, alla prima cosa che prova a fare; e una capacità nuova sarebbe
  stata una cosa in più da ricordarsi, cioè una regola che si perde alla prima
  riga scritta di fretta.
- **Il limite è dichiarato: un job che non chiama mai l'host arriva in fondo.**
  ...
  cancellazione è cooperativa **perché non può essere altro**, e la risposta
  vera per il codice che non collabora arriva a M5, dove un componente WASM ha
  un deadline. Dirlo qui è la differenza fra un limite e una sorpresa: ha una
  prova sua, che verifica che quel job arrivi in fondo.
- **`Cancelled` è una variante nuova del contratto, e va aggiunta adesso.** È
  ...
  l'utente leggerebbe «errore interno del plugin» sotto un pulsante che ha
  appena premuto. È additiva (in coda al variant) e costa un campo oggi; dopo il
  freeze di M4 costerebbe una migrazione di versione, ed è il criterio che fa di
  questa mezza voce una P0.
- **Le capacità che non possono fallire non rifiutano**, e sono cinque:
  ...
  hanno **dove** metterlo, un rifiuto. Nessuna delle cinque cambia il vault, e
  la ragione non è fortuna: nel contratto **tutto ciò che cambia il vault può
...
- **Annullare un job che non è ancora partito vale quanto annullarne uno in
  ...
  chi arriva prima, il runner o chi annulla; rispondere «non lo conosco» farebbe
  di «annulla» una corsa che si perde proprio nel momento in cui la si vorrebbe
  vincere.

### La chiusura

- **Chi chiude aspetta chi ha già cominciato, dopo avergli detto di smettere.**
  L'ordine è quello della [0029](0029-chiudere-un-vault-e-chiuderli-tutti.md)
  con un gemello in mezzo: **prima si smette di guardare** (il watcher), **poi
  si smette di lavorare** (il pool), **poi si chiude**. Le prime due sono la
  stessa regola letta due volte — nessun altro thread deve poter entrare nel
  vault mentre lo si chiude — e non aspettare vorrebbe dire lasciare un job che
  scrive mentre gli indici si chiudono.
...
- **La rete sta attorno alla chiamata del provider, e a niente di più.** È la
  ...
  kernel, con la tabella delle view **vuota** o un comando per sempre "in giro"
  a rifiutarsi da sé.
- **Otto porte, e sono tutte quelle da cui si entra in codice di un plugin**:
  `invoke_command` (i due rami), `view_action`, `render_view`, `call_service`,
  la consegna a un `EventHandler`, l'alimentazione degli indici registrati, il
  `parse` di un `FormatProvider`, l'innesto di una `SyntaxRule` e il disegno di
  un `CustomRenderer`. Le ultime quattro sono dentro **ogni scrittura**, che è
  dove la voce diceva che il buco era rimasto aperto.
- **Un panico si traduce nell'errore di casa.** `PluginError::Internal` per chi
  ...
  renderer: così chi chiama lo tratta come tratta già il fallimento — il
  renderer che degradava al provider degrada anche qui, senza un ramo nuovo. E
  il messaggio **nomina** il colpevole: «qualcosa è andato storto» senza dire
  quale plugin è la stessa cosa che non dirlo.
...
- **Un panico non disattiva niente**, e la voce chiedeva il contrario
  ...
  spiegazione. Un panico costa la chiamata; il giorno che le due voci ci sono,
  la politica si decide con loro davanti.
- **Il veleno del lock resta un segnale, e non lo si aggira.** I
  `.expect("workspace avvelenato")` restano dove sono: da questa decisione in
  poi un lock avvelenato vuol dire che a paniare è stato **il kernel**, non un
...
- **Il registry teneva un `Box`, adesso tiene un `Arc`, e la
  ...
  chiude ferma il pool **prima**. E se un giorno qualcuno invertisse i due
  passi, quel commiato non verrebbe chiamato: il registry lo **dice** con un
  errore che nomina la causa, invece di aspettare in silenzio.
- **Il `pop` prima del `?` era già scritto giusto.** Le pile e le tabelle del
  ...
  invocare sé stesso», che è il genere di bugia che si insegue per un
  pomeriggio.
- **Il parse viene prima della scrittura**, quindi un formato che pania non
  muove nemmeno il disco: la mutazione resta atomica anche quando a fallire è un
...
- **`PluginError` non arriva al frontend come tipo.** Il mirror TS non lo
  ...
  nuova quindi non ha cambiato una riga del frontend — e questo è anche la
  misura di quanto il §12.2 (errori con codice e parametri) sia ancora tutto lì.
...
- **Nessun progresso, e nessun pulsante.** Un job non ha modo di dire «sono al
  ...
  degli eventi va con lui — la voce dice già che il progresso «sarà il canale
  più caldo». `Host::cancel_job` è la porta a cui quel pulsante si attaccherà, e
  oggi la usano i presidi.
- **Nessuna priorità, nessuna quota, nessuna coda per plugin.** Un plugin che
  accoda mille job li fa eseguire tutti, in ordine di arrivo. Il tetto è §24.2,
  e sceglierlo qui vorrebbe dire inventare una politica senza un caso davanti —
  lo stesso motivo per cui il §14.2 non ha ancora le impronte.
- **Il numero dei thread non è misurato.** È un default con una ragione, non un
  ...
  embedding).
- **Nessun riavvio automatico di un job**, e nessuna coda persistente: un job
  che non è partito perché il vault si chiudeva non riparte alla riapertura.
  Renderlo durevole vuol dire deciderne l'idempotenza, che è una proprietà di
  chi lo scrive e non dell'host.
...
- `cargo test --workspace` — **61 suite, 0 fallimenti**. Sono le 59 della
  ...
  - `fub-host/tests/il_runner.rs` — cinque prove: un job accodato che parte da
    ...
    due tempi, perché un test che aspetta un tempo fisso prova la macchina su
    cui gira;
...
- **Mirror TS invariato**: `UPDATE_MIRROR=1` su `ts_mirror_app` e `ts_mirror`
  non produce nessuna differenza — `PluginError` non attraversa l'IPC come tipo.
