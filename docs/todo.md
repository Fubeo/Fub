...
capacità strutturali, che il §1.4 voleva decidere una per una a verbale. Un
...
c'era bisogno di provare. **Il §1.4 le ha decise, e adesso sono migrate**:
`note.create`, `note.rename`, `note.trash`, `trash.restore`, `trash.empty` sono
comandi come gli altri, sei comandi Tauri sono spariti, e la regola del §4.2 vale
finalmente anche per le feature che toccano il vault.

*Resta fuori, dichiarato:* ~~i **comandi strutturali**~~ (fatti, §1.4); i
...
- [x] **Operazioni strutturali** (`create_document`, `rename_document`,
      ...
      capitoli 16, 17, 8.3, 7.2. `create_folder` **non entra** (§2.11).
...
- [x] `storage_*` volatile **tolto**: linea di base ritagliata, l'unica rottura
      del giro.
- [x] Punto di applicazione del permesso: **deciso e rimandato al §2.10**, con
      la ragione a verbale e il varco del §1.36 esteso a tutte le strutturali.
...
rinomina è un lotto (§1.12), quindi una capacità dell'`HostApi` può toccare N
...
il posto giusto — il §1.6 è «il canale dati» — e non lo è: il cestino **non è
...
simulazione potrebbe diventare reale invocando qualcuno — cioè il buco che il
§1.36 aveva appena chiuso. (b) **Non prende un `Actor`, e non lo riazzera** — la
domanda che il §1.18 aveva lasciato aperta. L'attore è chi ha *chiesto*, e
...
riconosce più chi ha chiesto è quella che si richiama da sola — il §1.18 letto
...
è il §2.10. La differenza con `CommandScope.writes` — che invece è vincolante —
...
dire inventare il registro dei manifest (§2.10) e il runtime che rende possibile
...
diagnosi che al §1.1 ha tenuto fuori un campo `trust` dai comandi).

Ciò che invece **è stato fatto adesso** è la metà che non richiede il registro:
il varco del §1.36 copre tutte e sei le strutturali. Un comando simulato, o che
...
qualcuno aggiungesse senza pensarci. Il giorno del §2.10, il rifiuto non va
...
del §1.3 dall'altra, allo store volatile a chiave→valore restava solo il caso
...
- **`create_folder`** — no, §2.11. Nel kernel le cartelle non esistono: una
  ...
  (`convertToFolder` è una rinomina). Quando il §2.11 darà alle cartelle un
  modello, questa sarà additiva.
- **Allegati/asset** (`read_asset`, `write_asset`, `list_assets`) — no, §2.2. Il
  ...
  linka: una capacità senza le compagne che la rendono usabile. Va col §2.2.
- **Rete** (`http_fetch`, «solo dentro un job») — no, e le due voci aperte vanno
  lette **insieme**: il §1.4 la voleva solo dentro un job, e il §1.21 dice che un
  ...
  dove può girare è l'unico posto che non può scrivere. Servono prima §1.21 (un
  lavoro lungo che vede il vault) perché sia utile e §2.10 (`network` letto da
  qualcuno) perché sia sicura. Due bloccanti, entrambi nominati; dopo, additiva.
- **Tempo differito** (`schedule_at`, `schedule_every`) — no, §2.4. Il kernel è
...
- **`notify(level, message)` e `progress(job, done, total)`** — no, e con un
  criterio che vale anche per la prossima. Il §1.36 aveva stabilito che il
  **consenso** non è una capacità perché questo host non può fermarsi a
  ...
  già esiste; il centro notifiche è il §3 e il progresso vuole i job del §1.21.
...
- **Leggere l'`Actor` corrente** — no, e la ragione è del §1.18: l'origine è ciò
  che l'host **appone**, non ciò che il comando legge; un comando che si
  comportasse diversamente a seconda di chi lo chiama sarebbe una policy (§2.10)
  nascosta dentro un'implementazione. Se un giorno servirà, è additiva.
- **`set_active_context`** — no, già deciso al §1.9 e confermato: quale nota
...
Tauri, ed è quella sparizione a rendere vera la regola del §4.2 («una feature
...
prosa: è il §1.11, e i test che discriminano per sottostringa sono lì a
ricordarlo.
...
- [ ] **Il varco però esiste già, e il §1.4 gliel'ha allargato**: un comando
...
- [ ] **La cartella come cittadino**: `metas` è una mappa piatta
      ...
      È anche la ragione per cui il §1.4 ha lasciato `create_folder` **fuori**
...
- [ ] **Toast/notification center** alimentato da un **evento**, non da una
      capacità: il §1.4 ha deciso che `notify` non è un metodo dell'`HostApi` —
...
- [ ] **Test che presidia la superficie**: l'elenco dei comandi Tauri
      (25 oggi, sei in meno dopo il §1.4) è una **allowlist** in un test; aggiungerne
...
- [ ] **Migrare i bespoke esistenti** dove il §1 lo rende possibile: versioning
      (3 comandi), ~~cestino (4)~~ e ~~organizzazione (2)~~ — fatti col §1.4:
      ...
      Grafo (1).
- [ ] **La riga che divide, trovata dal §1.4 e da tenere**: un comando fa
...
§1.1 comandi, §1.2 `UiNode` con input, ~~§1.4 capacità `HostApi`~~ (**fatto**),
§1.5 modello
