...
/// La frase di ogni permesso, con la propria **chiave di catalogo scritto per
...
export const TRUST_LABELS: Record<Trust, Key> = {
  core: "trust.core",
...
export function permissionKey(pluginId: string, permission: string): string {
  const i = permission.indexOf(":");
  const name = i < 0 ? permission : permission.slice(i + 1);
  return `${pluginId}:permissions.${name}`;
}
...
export function isPermissionKey(key: string): boolean {
  ...
  return name.startsWith("permissions.") && name.length > "permissions.".length;
}
...
///
/// I **noti** vengono per primi e nell'ordine di `PERMISSIONS`; gli altri in fondo,
...
export function rows(bundle: BundleInfo): PermissionRow[] {
  const declared = Object.entries(bundle.permissions).filter(
    ([, v]) => v !== false && v !== null,
  );
  const known = PERMISSIONS.filter((p) => declared.some(([k]) => k === p)).map((p) =>
    row(bundle, p, bundle.permissions[p], true),
  );
  const unknown = declared
    .filter(([k]) => !(PERMISSIONS as readonly string[]).includes(k))
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([k, v]) => row(bundle, k, v, false));
  return [...known, ...unknown];
}

function row(bundle: BundleInfo, permission: string, value: unknown, known: boolean): PermissionRow {
  return {
    permission,
    key: permissionKey(bundle.id, permission),
    message: known ? t(PHRASES[permission as Permission]) : t("permission.unknown"),
    detail: detailOf(permission, value),
    known,
  };
}
...
export function detailOf(permission: string, value: unknown): string | null {
  if (permission !== "fub:network") return null;
  const host = Array.isArray(value)
