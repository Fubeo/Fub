...
//
// La regola si presidia leggendo i sources perché è l'unico modo che non
...
//
// I sources si leggono con `import.meta.glob` di Vite e non con `node:fs`
...
const AUTHORIZED_MODULES = ["host/ipc.ts", "host/dialog.ts"];
...
function relativeToSrc(key: string): string {
  ...
  if (key.startsWith("./")) return `host/${key.slice(2)}`;
  return key;
}
...
describe("la cucitura con l'host", () => {
  it("è l'unica a importare @tauri-apps", () => {
    const offenders = Object.entries(sources)
      .map(([key, text]) => ({ file: relativeToSrc(key), text }))
      .filter(({ file }) => !AUTHORIZED_MODULES.includes(file))
      .filter(({ text }) => TAURI_IMPORT.test(text))
      .map(({ file }) => file);

    expect(offenders, `moduli che importano @tauri-apps fuori da ${AUTHORIZED_MODULES.join(", ")}`).toEqual(
      [],
    );
  });

  it("legge davvero i sources, e la lista delle eccezioni punta a file veri", () => {
    ...
    const file = Object.keys(sources).map(relativeToSrc);
    expect(file.length).toBeGreaterThan(10);
    for (const exception of AUTHORIZED_MODULES) expect(file).toContain(exception);
  });
  ...
  it("riconosce un'importazione Tauri quando c'è", () => {
    ...
    const authorizedModules = Object.entries(sources).filter(([k]) =>
      AUTHORIZED_MODULES.includes(relativeToSrc(k)),
    );
    ...
    expect(authorizedModules).toHaveLength(AUTHORIZED_MODULES.length);
    for (const [key, text] of authorizedModules) {
      expect(TAURI_IMPORT.test(text), relativeToSrc(key)).toBe(true);
    }
  });
});
