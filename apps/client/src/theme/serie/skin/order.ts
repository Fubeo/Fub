 ...
 * fuoco, salto al contenuto — stanno nella struttura (\`theme/structure.css\`),
 * che è della shell e non si tematizza. I valori — la scala, i ruoli di
 * colore, il moto — stanno nei fogli (\`sheet-dark.css\` /
 * \`sheet-light.css\`): questa pelle li consuma e non li dichiara.
 *
...
/// puntano dentro `skin.css`, e senza questa riga trovare il pezzo da toccare
/// vorrebbe dire cercare la regola a memoria.
function header(part: string): string {
  const headerPrefix = `/* ── skin/${part}.css `;
  return `${headerPrefix}${"─".repeat(Math.max(3, 76 - headerPrefix.length))} */\n`;
}

/// Monta la pelle dai suoi pezzi. `content` è il testo di ogni pezzo, per
/// nome; chiunque lo legga — da disco o da Vite — passa di qui.
export function assemble(content: Readonly<Record<string, string>>): string {
  ...
  const extraParts = Object.keys(content).filter((n) => !(ORDER as readonly string[]).includes(n));
  if (extraParts.length > 0) {
    throw new Error(
      `pelle: ${extraParts.map((n) => `«${n}.css»`).join(", ")} non è nell'ORDINE, e un pezzo ` +
  ...
  for (const part of ORDER) {
    ...
    if (text === undefined) throw new Error(`pelle: manca il pezzo «${part}.css»`);
    outside.push(`\n${header(part)}\n${text.replace(/\n+$/, "\n")}`);
  }
  return outside.join("");
}
