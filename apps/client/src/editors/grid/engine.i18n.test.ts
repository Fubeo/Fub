...
vi.mock("../../i18n/strings", () => ({
  ...
  t(key: string) {
    const messages = {
      it: { "grid.a11y.superficie": "Foglio di calcolo" },
      en: { "grid.a11y.superficie": "Spreadsheet" },
    } as const;
    return messages[i18n.language as keyof typeof messages]?.[key as "grid.a11y.superficie"] ?? key;
  },
}));
