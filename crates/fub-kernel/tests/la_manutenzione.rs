...
use fub_kernel::maintenance::{
    Maintenance, MAINTENANCE_ID, VAULT_DIAGNOSTIC_BUNDLE, VAULT_REBUILD_INDEX, VAULT_REPAIR,
};
...
/// I tre sono nel registro **come tutti gli altri**: è ciò che li rende
...
/// direbbe.
#[test]
fn i_tre_comandi_stanno_nel_registro() {
    let (_dir, _root, ws) = vault();
    let ids: Vec<String> = ws.commands().into_iter().map(|s| s.id).collect();
    for atteso in [VAULT_REBUILD_INDEX, VAULT_REPAIR, VAULT_DIAGNOSTIC_BUNDLE] {
