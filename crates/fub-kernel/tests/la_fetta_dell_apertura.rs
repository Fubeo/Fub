...
impl IndexProvider for SpiaIndice {
    ...
    fn on_documents_indexed(&mut self, docs: &[DocumentModel]) -> Vec<IndexLoss> {
        let mut testi = self.testi.lock().unwrap();
        for doc in docs {
            testi.push(doc.text.clone());
        }
        Vec::new()
    }
...
fn voce(ws: &fub_kernel::Workspace, id: &DocId) -> VaultEntry {
...
#[test]
fn un_piano_invecchiato_non_cancella_chi_ha_salvato_durante_l_apertura() {
    let (mut banco, spia) = banco_da_aprire();
    let id = DocId::new("nota.md");
    banco.scrivi("nota.md", "dal disco\n");

    let mut work = banco.scan_vault().expect("la scansione riesce");
    // Fase 1, sotto prestito condiviso: la fetta legge «dal disco».
    let piano = banco.plan_batch(&mut work);
    ...
    let testi = spia.testi.lock().unwrap().clone();
    assert_eq!(
        testi.last().map(String::as_str),
        Some("dall'utente\n"),
        "l'ultimo testo arrivato all'indice è quello letto dal disco *prima* \
         ...
         niente lo dice fino alla riapertura ({testi:?})"
    );
}
...
/// accorga.
#[test]
fn senza_nessuno_in_mezzo_la_fetta_entra_intera() {
    let (mut banco, spia) = banco_da_aprire();
    for n in 0..3 {
        banco.scrivi(&format!("Nota{n}.md"), &format!("corpo {n}\n"));
    }
    ...
    let mut testi = spia.testi.lock().unwrap().clone();
    testi.sort();
    assert_eq!(
        testi,
        ["corpo 0\n", "corpo 1\n", "corpo 2\n"],
    ...
    assert_eq!(
        voce(&banco, &DocId::new("Nota1.md")).fingerprint,
