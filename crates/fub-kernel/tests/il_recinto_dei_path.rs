...
impl Banco {
    ...
    fn scrivi(&self, rel: &str, body: &str) {
...
#[test]
fn una_cancellazione_nel_cestino_non_risale_fuori() {
    let banco = Banco::nuovo();
    banco.scrivi("fuori.txt", "il file di qualcun altro");
    banco.scrivi(".trash/Idea.txt", "cestinata");
    let vault = banco.vault();

    let esito = vault.remove_trashed(&DocId::new(".trash/../fuori.txt"));
    ...
    assert!(
        banco.root.join("fuori.txt").exists(),
...
/// fuori dalla radice.
#[test]
fn un_ripristino_non_atterra_fuori_dalla_radice() {
    let banco = Banco::nuovo();
    banco.scrivi(".trash/Idea.txt", "cestinata");
    let vault = banco.vault();

    let esito = vault.restore_trashed(&DocId::new(".trash/Idea.txt"), &DocId::new("../fuori.txt"));

    assert!(esito.is_err(), "una destinazione che risale si rifiuta");
    assert!(
        !banco
            .root
            .parent()
    ...
    assert!(
        banco.root.join(".trash/Idea.txt").exists(),
...
#[test]
fn nessun_path_composto_da_questo_vault_esce_dalla_radice() {
    let banco = Banco::nuovo();
    banco.scrivi("fuori.txt", "il file di qualcun altro");
    let vault = banco.vault();
...
#[test]
fn un_ripristino_non_atterra_dove_la_scansione_non_guarda() {
    let banco = Banco::nuovo();
    banco.scrivi("Idea.txt", "un'idea");
    let mut ws = banco.workspace();
    let cestinata = ws.delete_document(&DocId::new("Idea.txt")).unwrap();

    let err = ws
        .restore_from_trash(&cestinata, Some(DocId::new(".nascosta/Idea.txt")))
        .unwrap_err();

    assert!(err.to_string().contains("nome non valido"), "{err}");
    assert!(
        !banco.root.join(".nascosta").exists(),
...
/// già.
#[test]
fn un_ripristino_al_suo_posto_riporta_anche_un_nome_che_oggi_non_nascerebbe() {
    let banco = Banco::nuovo();
    banco.scrivi("CON.txt", "un nome che su Windows non si crea");
    let mut ws = banco.workspace();
    let cestinata = ws.delete_document(&DocId::new("CON.txt")).unwrap();

    let tornata = ws
        .restore_from_trash(&cestinata, None)
        .expect("il nome c'era già: torna");

    assert_eq!(tornata, DocId::new("CON.txt"));
    assert!(banco.root.join("CON.txt").exists());
}
...
#[test]
fn una_chiave_scritta_a_mano_passa_dal_recinto_come_ogni_altro_path() {
    let banco = Banco::nuovo();
    banco.scrivi(
        ".fub/workspace.json",
        r#"{
          ...
          "spaces": [".fub"],
          "order": {"": ["../fuori.txt", "Nota.txt"]}
        }"#,
    );

    let (store, guasto) = OrganizationStore::open(&banco.root, Arc::new(FsStorage));

    assert!(guasto.is_none(), "il file si legge: {guasto:?}");
    ...
    assert_eq!(avvisi.len(), 1, "e qualcuno lo dice: {avvisi:?}");
    assert!(
        avvisi[0].contains("non stanno in questo vault"),
        "{}",
        avvisi[0]
    );
}
