...
#[test]
fn open_and_resave_not_changes_a_byte() {
    ...
    for (name, bytes) in corpus() {
        ...
        let after = std::fs::read(vault.path_for(&id).unwrap()).expect("rilegge");
        assert_eq!(
            after,
            bytes,
            "{name}: apri-e-salva ha cambiato i byte del file\n  prima: {:?}\n  dopo:  {:?}",
...
#[test]
fn the_read_not_removes_the_bom_of_it_converts_the_terminators() {
    ...
    .unwrap();
    let read_value = vault.read(&id).unwrap();

    assert_eq!(text_policy::bom_len(&read_value), 3, "il BOM è stato tolto");
    assert_eq!(
        Newline::of(&read_value),
        Newline::Crlf,
        "i terminatori sono stati convertiti"
    );
    assert!(read_value.starts_with('\u{feff}'));
    // E gli offset sono quelli del file: il primo byte di contenuto è il quarto.
    assert_eq!(&read_value[3..11], "# Titolo");
}

#[test]
fn a_file_that_not_and_utf8_says_a_which_byte_the_stops() {
    let (_dir, vault) = tempvault();
    ...
    assert!(
        text.contains("non è UTF-8") && text.contains(" a 4"),
        "l'errore deve dire a quale byte: {text}"
    );
    assert!(text.contains("latin1.md"), "e su quale file: {text}");
...
#[test]
fn the_byte_raw_remain_readable_also_when_the_text_no() {
