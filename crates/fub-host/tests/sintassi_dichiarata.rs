...
fn forme() -> Vec<SyntaxForm> {
    ...
    // committato.
    let montato = fub_host::mount::mount(
        &root,
    ...
    montato.workspace.syntax_forms(&DocId::new(SONDA))
}

fn trigger_ts(t: &SyntaxTrigger) -> String {
    match t {
        SyntaxTrigger::Fence { info } => {
            let voci: Vec<String> = info.iter().map(|i| format!("{i:?}")).collect();
            format!("{{ fence: {{ info: [{}] }} }}", voci.join(", "))
        }
...
#[test]
fn la_dichiarazione_emessa_e_quella_del_montaggio() {
    let emesso = render();
    let path = path();

    if std::env::var_os("UPDATE_MIRROR").is_some() {
        std::fs::write(&path, &emesso).expect("scrive la dichiarazione generata");
        return;
    }

    let committato = std::fs::read_to_string(&path).unwrap_or_else(|e| {
        panic!(
            "dichiarazione generata mancante ({}): {e}. Rigenerala con \
             `UPDATE_MIRROR=1 cargo test -p fub-host --test sintassi_dichiarata`.",
            path.display()
        )
    });
    ...
    assert_eq!(
        emesso, committato,
        "`frontend/src/rules/sintassi.generated.ts` è stantio: la sintassi \
         dichiarata dal montaggio è cambiata senza rigenerarlo.\nRigenera con \
         `UPDATE_MIRROR=1 cargo test -p fub-host --test sintassi_dichiarata`."
    );
}
...
/// funzionare meglio.
#[test]
fn le_sintassi_senza_forma_dichiarata_sono_quelle_del_provider() {
    let forme = forme();
    let senza: BTreeSet<&str> = forme
        .iter()
    ...
    assert_eq!(
        ...
        "le sintassi che il provider markdown conosce come grammatica sono \
         cambiate. Se una ha guadagnato un trigger dichiarato, la shell adesso \
         la può GENERARE invece di riscriverla: toglila da qui e togli la sua \
         regex da `frontend/src/rules/sintassi.ts`. Se ne è nata una senza \
         trigger, la shell dovrà riscriverla a mano — che è il moltiplicatore \
         del §4.4, e va deciso invece che subito"
    );

    let con: BTreeSet<&str> = forme
        .iter()
    ...
    assert_eq!(
        ...
        "le sintassi con forma dichiarata sono cambiate"
    );
}
