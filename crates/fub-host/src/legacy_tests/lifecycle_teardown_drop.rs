...
impl Probe {
    fn drop_resource(&self, resource: Resource) {
        ...
        let workspace_write = workspace.write().is_ok();
...
impl Bundle for DropBundle {
    ...
    fn register(&self, registrar: &mut Registrar<'_>) -> Vec<String> {
        registrar
            .register_index_provider(Box::new(DropIndex(self.0.clone())))
            .expect("register index");
        for resource in [Resource::FirstHandler, Resource::SecondHandler] {
            registrar
                .register_event_handler(Box::new(DropHandler {
                    resource,
                    probe: self.0.clone(),
                }))
        ...
        registrar
            .register_command_provider(Box::new(DropCommand(self.0.clone())))
...
fn exercise(path: Path, panic_at: Option<Resource>) {
    ...
    });
    {
        ...
        if path.is_manual() {
            ...
            ws.register_index_provider(OWNER, Box::new(DropIndex(probe.clone())))
                .expect("register manual index");
            for resource in [Resource::FirstHandler, Resource::SecondHandler] {
                ...
                .expect("register manual event handler");
            }
            ws.register_command_provider(OWNER, Box::new(DropCommand(probe.clone())))
                .expect("register manual command");
        } else {
            registry
                .write()
                .unwrap()
