...
//! # fn esempio(mio: &mut impl fub_abi::traits::IndexProvider) {
...
pub fn cio_che_non_e_perduto_si_ritrova<I: IndexProvider + ?Sized>(
    ...
    docs: &[DocumentModel],
    query: IndexQuery,
    ritrovato: impl Fn(&fub_abi::traits::IndexResult, &DocId) -> bool,
) -> bool {
...
pub fn ogni_view_dichiarata_si_disegna<V: ViewProvider + ?Sized>(view: &V, host: &dyn ReadApi) {
    for spec in view.views() {
        let istanza = fub_abi::traits::ViewInstance::only(spec.id.clone());
