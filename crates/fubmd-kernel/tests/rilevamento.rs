...
//! 0030).
//!
//! Il watcher è l'unico meccanismo con cui FubMD viene a sapere che qualcun
...
use fubmd_abi::traits::{IndexQuery, IndexResult, VaultStatus};
...
impl TempDir {
    fn new(tag: &str) -> Self {
        let base = Utf8PathBuf::from_path_buf(std::env::temp_dir())
            ...
            .join(format!("fubmd-rilevamento-{tag}-{}", std::process::id()));
