...
const CAMPI: &[&str] = &["pane", "doc", "selections", "mode"];

#[test]
fn il_contesto_di_sessione_ha_quattro_campi_e_nessun_bersaglio() {
    ...
    let mut trovati: Option<Vec<String>> = None;
    ...
    let trovati = trovati.expect("`struct ViewContext` sta in src/session.rs");
    assert_eq!(
        trovati, CAMPI,
