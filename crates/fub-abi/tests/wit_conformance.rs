...
use fub_abi::grid::{
    GridApplyRequest, GridCell, GridCellKey, GridCellPatch, GridCellSnapshot, GridCellStyle,
    GridCellValue, GridColumn, GridCommit, GridFormulaError, GridHorizontalAlign, GridInvalidation,
    GridProvider, GridRow, GridSession, GridSheet, GridSurfaceSpec, GridWindow, GridWindowRequest,
};
...
fn conform(source: &str) -> Result<(), String> {
    ...
    let GridSession { instance, sheets } = GridSession {
        instance: String::new(),
        sheets: Vec::new(),
    };
    contract.record(
        "grid-session",
        &[("instance", wit(&instance)), ("sheets", wit(&sheets))],
    );
    ...
    let GridCellSnapshot { input, style } = GridCellSnapshot {
        input: String::new(),
        style: GridCellStyle::default(),
    };
    contract.record(
        "grid-cell-snapshot",
        &[("input", wit(&input)), ("style", wit(&style))],
    );
    ...
    let GridApplyRequest { patches } = GridApplyRequest {
        patches: Vec::new(),
    };
    contract.record("grid-apply-request", &[("patches", wit(&patches))]);
    ...
    let GridCommit { edit, invalidation } = GridCommit {
        edit: EditRequest::new(Revision::of(""), Vec::new()),
        invalidation: GridInvalidation::All,
    };
    contract.record(
        "grid-commit",
        &[("edit", wit(&edit)), ("invalidation", wit(&invalidation))],
    );
