...
#[test]
fn that_that_returns_and_that_that_was_exited() {
    for (name, src) in cases() {
        let rewritten = serialize(&parse(src));
        assert_eq!(
            rewritten, src,
            "«{name}»: il giro ha riscritto il file dell'utente diverso da\n\
...
#[test]
fn the_second_round_and_stopped() {
    for (name, src) in cases() {
        let one = serialize(&parse(src));
