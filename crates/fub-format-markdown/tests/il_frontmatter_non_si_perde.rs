...
#[test]
fn il_frontmatter_illeggibile_sopravvive_al_giro_completo() {
    for (caso, sorgente) in ILLEGGIBILI {
        let modello = parse(sorgente);
        let riscritto = serialize(&modello);

        assert_eq!(
            &riscritto.as_str(),
            sorgente,
            "«{caso}»: il giro completo ha riscritto il documento senza il suo\n\
        ...
        assert_eq!(
            serialize(&parse(&riscritto)),
            riscritto,
            "«{caso}»: il secondo giro non coincide col primo.",
        );
    }
}
...
#[test]
fn il_blocco_conservato_dice_cosa_conserva_e_perche() {
    for (caso, sorgente) in ILLEGGIBILI {
        let modello = parse(sorgente);
        ...
        let Some(Block::Custom {
            ...
            panic!("«{caso}»: il frontmatter illeggibile non è nel modello");
        };
        assert_eq!(kind, custom_kind::FRONTMATTER_UNPARSED, "«{caso}»");
        ...
        let text = attrs
            ...
            .unwrap_or_default();
        assert!(
            sorgente.starts_with(text),
            "«{caso}»: il testo conservato non è quello del file:\n{text:?}",
        );
        ...
        assert_eq!(
            sorgente[span.start..span.end].trim_end(),
            text.trim_end(),
            "«{caso}»: lo span non nomina il frontmatter",
        );
        ...
        assert!(
            ...
            "«{caso}»: nessun motivo: l'utente vedrebbe le proprietà svanire\n\
        ...
        let html = MarkdownProvider::new()
            .render_html(&modello, &Default::default())
            .expect("la resa riesce");
        assert!(
            html.contains("block-frontmatter-unparsed") && html.contains("<pre>"),
            "«{caso}»: il frontmatter illeggibile non compare nell'anteprima:\n{html}",
        );
    }
}
...
#[test]
fn il_frontmatter_leggibile_non_passa_dal_ramo_nuovo() {
    let modello = parse("---\ntitolo: Nota\ntags:\n  - a\n---\n\nCorpo.\n");
    assert_eq!(
        modello.frontmatter.get("titolo").and_then(|v| v.as_str()),
        Some("Nota")
    );
    assert!(
        !modello.body.iter().any(|b| matches!(
            b,
    ...
    assert!(serialize(&modello).starts_with("---\ntitolo: Nota\n"));
}
...
#[test]
fn il_serializer_non_ingoia_i_propri_fallimenti() {
    const SORGENTE: &str = include_str!("../src/serialize.rs");

    assert!(
        SORGENTE.contains("pub fn serialize(model: &DocumentModel) -> Result<String, FormatError>"),
    ...
    for forma in ["if let Ok(", "let Ok(", ".ok()", ".unwrap_or_default()"] {
        for (n, riga) in SORGENTE.lines().enumerate() {
            ...
            assert!(
                !riga.contains(forma),
                "src/serialize.rs:{}: `{forma}` scarta un fallimento invece di\n\
...
#[test]
fn la_forma_del_frontmatter_non_fa_fallire_lo_yaml() {
    ...
    let mut mappa = ostili.as_object().expect("è un oggetto").clone();
    ...
    mappa.insert("profondo".to_string(), profondo);

    let esito = serde_yaml_ng::to_string(&mappa);
    assert!(
        esito.is_ok(),
