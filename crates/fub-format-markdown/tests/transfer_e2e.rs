...
fn artifact<'a>(
    report: &'a fub_abi::transfer::ExportReport,
    path: &str,
) -> &'a fub_abi::transfer::ExportArtifact {
    report
        .artifacts
        .iter()
        .find(|a| a.path == path)
...
fn what_goes_out_comes_back_in_identical() {
    ...
    // kernel non presta capacità a una stringa.
    altro
        .register_core_feature("fub.markdown", "fub.markdown")
        .expect("dichiarato");
    altro
        .register_import_provider("fub.markdown", MarkdownImport::boxed())
        .expect("registrato");
