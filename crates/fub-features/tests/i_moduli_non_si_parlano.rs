...
const RADICE: &[&str] = &["lib.rs", "inventario.rs"];
...
fn solo_codice(sorgente: &str) -> String {
    let byte: Vec<char> = sorgente.chars().collect();
    let mut fuori = String::with_capacity(sorgente.len());
    let mut i = 0;
    while i < byte.len() {
        let c = byte[i];
        let dopo = byte.get(i + 1).copied();
        match (c, dopo) {
            // Commento di riga: fino al newline, che si tiene.
            ('/', Some('/')) => {
                while i < byte.len() && byte[i] != '\n' {
                    fuori.push(' ');
                    i += 1;
                }
            }
            ...
            ('/', Some('*')) => {
                let mut livello = 0usize;
                while i < byte.len() {
                    if byte[i] == '/' && byte.get(i + 1) == Some(&'*') {
                        livello += 1;
                        fuori.push_str("  ");
                        i += 2;
                    } else if byte[i] == '*' && byte.get(i + 1) == Some(&'/') {
                        livello -= 1;
                        fuori.push_str("  ");
                        i += 2;
                        if livello == 0 {
...
/// avere un soggetto.
fn moduli_di_feature() -> Vec<(String, String)> {
    let src = concat!(env!("CARGO_MANIFEST_DIR"), "/src");
    let mut fuori = Vec::new();
    for voce in std::fs::read_dir(src).expect("il crate ha una cartella `src`") {
        let percorso = voce.expect("una voce leggibile di `src`").path();
        let Some(nome) = percorso.file_name().and_then(|n| n.to_str()) else {
            continue;
        };
        if !nome.ends_with(".rs") || RADICE.contains(&nome) {
            continue;
        }
        let sorgente = std::fs::read_to_string(&percorso)
            .unwrap_or_else(|e| panic!("«{nome}» non si legge: {e}"));
        fuori.push((nome.to_string(), sorgente));
    }
    fuori.sort();
    assert!(
        !fuori.is_empty(),
...
#[test]
fn nessun_modulo_di_feature_nomina_la_radice() {
    let mut colpevoli: BTreeSet<String> = BTreeSet::new();
    for (nome, sorgente) in moduli_di_feature() {
        for (n, riga) in solo_codice(&sorgente).lines().enumerate() {
            if riga.contains("crate::") {
                colpevoli.insert(format!("  {nome}:{} → {}", n + 1, riga.trim()));
            }
        }
    }
    ...
    assert!(
        ...
        colpevoli.into_iter().collect::<Vec<_>>().join("\n")
    );
}
...
#[test]
fn lo_scanner_non_conta_la_prosa_e_non_si_lascia_sfuggire_il_codice() {
    let finto = r#"
...
"#;
    ...
    let codice = solo_codice(finto);
    let trovate: Vec<&str> = codice
        .lines()
    ...
    assert_eq!(
        trovate.len(),
        2,
        "lo scanner ha trovato {} righe invece di 2:\n{trovate:#?}\n\
         (le due vere sono l'`use` dopo il blocco chiuso a metà riga e quello \
          dietro il `#[cfg]`; tutto il resto è prosa o stringhe)",
        trovate.len()
    );
    assert!(
        trovate[0].contains("crate::outline::OUTLINE_ID"),
        "il `use` che segue un commento a blocco chiuso a metà riga deve \
         sopravvivere: è il caso in cui saltare troppo costa un falso verde"
    );
    assert!(
        trovate[1].contains("crate::search::SearchIndex"),
        "l'import guardato da `#[cfg]` deve sopravvivere: è **la** forma che il \
         compilatore non prende, ed è la ragione per cui questo banco esiste"
    );
}
