...
const COMMON: &str = "aghifoglia";

/// Un indice vero — tantivy su disco — con `quante` note che combaciano tutte.
fn index(count: usize) -> (tempfile::TempDir, SearchIndex) {
    ...
    let notes: Vec<DocumentModel> = (0..count)
        .map(|the| {
            let mut m = DocumentModel::empty(DocId::new(format!("nota{the:05}.md")));
            m.text = format!("Nota {the}. Un corpo qualsiasi che nomina una {COMMON}.");
            m
        })
        .collect();
    let _ = idx.on_documents_indexed(&notes);
    (dir, idx)
}

/// La domanda come la fa il pianificatore: niente estratti, e la finestra è
/// l'unica cosa che cambia fra un caso e l'altro.
fn ask(idx: &SearchIndex, page: Option<Page>) -> (usize, u32) {
    match idx.query(IndexQuery::Documents {
        matching: QueryExpr::of(QueryPredicate::Text(TextQuery::terms(COMMON))),
        sort: None,
...
#[test]
fn unpaged_searches_return_all_results() {
    let (_g, idx) = index(2000);

    let (count, total) = ask(&idx, None);

    assert_eq!(
        count, 2000,
        "senza finestra sono uscite {count} righe su duemila: qualcuno ha messo un tetto"
    );
    assert_eq!(total, 2000, "e il conteggio deve dire lo stesso numero");
}
...
fn a_window_does_not_pays_the_vault() {
    ...
    let _ = ask(&mille, Some(Page::first(20)));
    let _ = ask(&duemila, Some(Page::first(20)));

    let a = allocations_of(|| ask(&mille, Some(Page::first(20))));
    let b = allocations_of(|| ask(&duemila, Some(Page::first(20))));

    assert_eq!(
        ask(&mille, Some(Page::first(20))).0,
        ask(&duemila, Some(Page::first(20))).0,
