...
fn copertura(surface: ViewSurface) -> Copertura {
    match surface {
        ViewSurface::LeftSidebar => Copertura::Dogfooding,
        ViewSurface::RightSidebar => Copertura::Dogfooding,
        ViewSurface::Main => Copertura::Dogfooding,
        ViewSurface::StatusBar => Copertura::Dogfooding,
        ViewSurface::Bottom => Copertura::Scoperta(
            "la shell la ospita, ma nessuna feature ufficiale ci sta: la \
             console dei job e i risultati di ricerca in fondo sono voci di \
             piano, non codice",
        ),
        ViewSurface::Modal => Copertura::Scoperta(
...
/// nuova.
#[test]
fn il_dogfooding_dichiara_fin_dove_arriva() {
    let esercitate: Vec<ViewSurface> = fub_features::ogni_view_ufficiale()
    ...
    assert!(
        !esercitate.is_empty(),
        "nessuna view ufficiale dichiara una superficie: un presidio che itera \
         zero elementi passa sempre"
    );

    for surface in esercitate {
        if let Copertura::Scoperta(perche) = copertura(surface) {
            panic!(
                "una feature ufficiale sta su `{surface:?}`, che è dichiarata \
                 scoperta ({perche}). È una buona notizia scritta nel posto \
    ...
    let scoperte = ViewSurface::ALL
        .iter()
        .filter(|&&s| matches!(copertura(s), Copertura::Scoperta(_)))
        .count();
    assert_eq!(
        scoperte, 6,
