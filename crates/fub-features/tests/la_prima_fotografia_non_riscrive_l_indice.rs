...
//!
//! **Rosso**: `la_prima_fotografia_scrive_l_indice_una_volta_sola`. Con la forma
...
fn la_prima_fotografia_scrive_l_indice_una_volta_sola() {
    ...
    let (volte, byte) = host.scritture_su(INDEX_FILE);
    assert_eq!(
        volte, 1,
        "l'indice è stato scritto {volte} volte per {NOTE} note: la passata \
    ...
    let sul_disco = host.data_read(INDEX_FILE).expect("lettura").expect("c'è");
    assert_eq!(
        byte,
        sul_disco.len(),
...
fn l_indice_scritto_una_volta_sola_dice_tutto_quello_che_diceva_prima() {
    ...
    assert_eq!(riaperto.documents().len(), NOTE);
    for id in riaperto.documents() {
        let versioni = riaperto.list(&id);
        assert_eq!(versioni.len(), 1, "{id} ha {} versioni", versioni.len());
        assert!(
            riaperto.read(&id, versioni[0].ts, &host).is_ok(),
