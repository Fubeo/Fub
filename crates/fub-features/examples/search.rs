...
fn documents(matching: QueryExpr, page: Option<Page>) -> IndexQuery {
...
fn count_results(ws: &Workspace, q: IndexQuery) -> (usize, u32) {
...
fn row(name: &str, ms: f64, resi: usize, total: u32) {
    println!("{name:<46} {ms:>9.2} ms   {resi:>4} resi / {total} combaciano");
}
...
fn main() {
    ...
    for limit in [1u32, 5, 20, 50, 100] {
        let q = || documents(text_query("concorrenza"), Some(Page::first(limit)));
        let (resi, tot) = count_results(&ws, q());
        let ms = median(|| drop(ws.query_index(q())));
        row(&format!("termine comune, page {limit}"), ms, resi, tot);
    }
    let q = || documents(text_query("ittiosauro"), Some(Page::first(20)));
    let (resi, tot) = count_results(&ws, q());
    let ms = median(|| drop(ws.query_index(q())));
    row("termine selettivo (1 nota), page 20", ms, resi, tot);
    ...
    let (resi, tot) = count_results(&ws, q());
    let ms = median(|| drop(ws.query_index(q())));
    row("tag:rust (nessun estratto), page 20", ms, resi, tot);
    ...
    let (resi, tot) = count_results(&ws, q());
    let ms = median(|| drop(ws.query_index(q())));
    row("solo nome \"Nota 7\", page 20", ms, resi, tot);
    ...
    for (name, ex) in [
        ...
        let q = || IndexQuery::Documents {
            matching: text_query("concorrenza"),
            sort: None,
            select: PropertySelect::None,
            page: None,
            excerpts: ex,
    ...
    phases(&index_dir, "concorrenza", 20);
    ...
    let for_keystroke = |text: &str, fields: Vec<TextField>, ex: Excerpts| {
        let q = || IndexQuery::Documents {
            matching: QueryExpr::of(QueryPredicate::Text(TextQuery {
                fields: fields.clone(),
                ..TextQuery::terms(text).while_typing()
            })),
            sort: None,
            select: PropertySelect::None,
    ...
    let word = "nota 1";
    for the in 1..=word.len() {
        let prefix = &word[..the];
        let (ms, resi, tot) = for_keystroke(prefix, vec![TextField::Name], Excerpts::Omit);
        row(
            &format!("solo nome, senza estratti: \"{prefix}\""),
            ms,
            resi,
            tot,
        );
    ...
    for the in 1..=word.len() {
        let prefix = &word[..the];
        let (ms, resi, tot) = for_keystroke(prefix, Vec::new(), Excerpts::Attach);
        row(
            &format!("ovunque, con estratti:     \"{prefix}\""),
            ms,
            resi,
            tot,
        );
    ...
    // processo.
    let list = || IndexQuery::Entries {
        of_kind: Some(EntryKind::Document),
        within: None,
        page: None,
    };
    let count = match ws.query_index(list()) {
        Ok(IndexResult::Entries(paged)) => paged.items.len(),
        other => panic!("risposta inattesa: {other:?}"),
    };
    let ms = median(|| drop(ws.query_index(list())));
    row(
    ...
    for prefix in ["c", "co", "conc", "concorrenza"] {
        let (ms, resi, tot) = for_keystroke(prefix, vec![TextField::Name], Excerpts::Omit);
        row(
            &format!("solo nome, termine comune: \"{prefix}\""),
            ms,
            resi,
            tot,
        );
...
fn phases(dir: &Utf8Path, term: &str, limit: usize) {
    let index = Index::open_in_dir(dir.as_std_path()).expect("indice su disco");
    let schema = index.schema();
    let field = |name: &str| schema.get_field(name).expect(name);
    ...
    let build_query = || -> Box<dyn Query> {
        let for_field: Vec<(Occur, Box<dyn Query>)> = [
        ...
        Box::new(BooleanQuery::new(vec![(
            Occur::Must,
            Box::new(BooleanQuery::new(for_field)) as Box<dyn Query>,
        )]))
    };
    ...
    let ms_searcher = median(|| drop(reader.searcher()));
    let ms_construction = median(|| drop(build_query()));
    let ms_count = median(|| {
        let _ = searcher.search(&*query, &Count).unwrap();
    });
    let collector = || TopDocs::with_limit(limit).and_offset(0).order_by_score();
    let ms_top = median(|| drop(searcher.search(&*query, &collector()).unwrap()));
    ...
    for (name, ms) in [
        ...
        ("costruzione della query", ms_construction),
        ...
        println!("{name:<46} {ms:>9.3}");
    }
    println!(
        "{:<46} {:>9.3}",
        "somma delle fasi",
        ms_searcher + ms_construction + ms_count + ms_top + ms_gen + ms_doc + ms_snip
    );
}
