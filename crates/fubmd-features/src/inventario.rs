...
//!
//! La tentazione era metterlo in `fubmd-testkit`, dove stanno le altre comodità
//! dei presidi. Non si può, e la ragione non è di gusto:
//! `fubmd-abi/tests/dependency_invariant.rs::il_banco_di_prova_non_entra_in_nessuna_libreria`
//! vieta a chiunque di dichiarare il banco fra le dipendenze **normali**, e
//! `fubmd_host::mount` è codice di libreria. Un inventario nel banco sarebbe
...
//!
//! `fubmd_host::mount` non elenca più le feature — le **itera da qui**, in
...
//! `fubmd-host/tests/le_view_ufficiali.rs` monta un workspace vero e confronta
...
//! soluzione.
//!
//! Il core (`fubmd.core`) **non** è qui, e non è una dimenticanza: le sue
//! impostazioni e il suo catalogo stanno in `fubmd-host` accanto allo schema che
...
/// (vedi `fubmd_host::mount`), e l'ordine delle view è quello in cui i pannelli
...
///
/// Chi la chiama: `fubmd_host::mount`, che ne fa i bundle, e i presidi che prima
