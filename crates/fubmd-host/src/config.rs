...
//! `FUBMD_CONFIG_DIR` resti una variabile d'ambiente mentre il §11.1 le sta
...
//!
//! 1. `FUBMD_CONFIG_DIR`, se c'è: è la porta di chi esegue una suite di test,
//!    di chi ne tiene due installazioni, e di chi impacchetta.
//! 2. Il **portable**: un file `fubmd.portable` accanto all'eseguibile fa
//!    scrivere la configurazione lì di fianco (`fubmd-config/`), invece che nel
...
pub fn config_dir() -> Option<Utf8PathBuf> {
    if let Some(dir) = env_path("FUBMD_CONFIG_DIR") {
        return Some(dir);
    }
...
mod tests {
    ...
    fn la_variabile_di_bootstrap_vince_su_tutto() {
        ...
        unsafe { std::env::set_var("FUBMD_CONFIG_DIR", "/tmp/fubmd-prova") };
        assert_eq!(config_dir(), Some(Utf8PathBuf::from("/tmp/fubmd-prova")));
        unsafe { std::env::remove_var("FUBMD_CONFIG_DIR") };
    }

    #[test]
    fn una_variabile_vuota_non_conta_come_una_scelta() {
        let _guardia = ENV.lock().unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("FUBMD_CONFIG_DIR", "   ") };
