//! Ciò che rende [`fubmd_features::ogni_feature_ufficiale`] **la sorgente**
...
//!   `fubmd.core`, che è dell'host e non di `fubmd-features` (le sue chiavi
...
//! fa restando verde. Nessuno dei due si vede da dentro `fubmd-features`, che il
...
use fubmd_host::CORE_ID;
use fubmd_kernel::{MachineSettings, SystemLocale, ViewStates};
...
fn monta(root: &Utf8PathBuf) -> fubmd_host::Mounted {
    fubmd_host::mount::mount(
        root,
...
fn i_bundle_dichiarati_sono_linventario_piu_il_core() {
    ...
    let attesi: BTreeSet<String> = fubmd_features::ogni_feature_ufficiale()
        .iter()
    ...
    assert!(
        ...
        "questi bundle sono dichiarati e l'inventario non li nomina: {di_troppo:?}\n\
         ...
         `fubmd_features::ogni_feature_ufficiale`, e da adesso i presidi che \
    ...
    // `fubmd-features`, l'asserzione sopra lo direbbe, ma vale la pena che il
    // conto sia scritto — è il modo in cui questo file dice quanto è larga
    // l'eccezione che si concede.
    assert_eq!(
        dichiarati.len(),
        fubmd_features::ogni_feature_ufficiale().len() + 1,
        "l'unico bundle che non è una feature di `fubmd-features` è `{CORE_ID}`"
    );
}
...
fn le_view_montate_sono_esattamente_quelle_dellinventario() {
    ...
    let promesse: BTreeSet<String> = fubmd_features::ogni_view_ufficiale()
    ...
    assert!(
        ...
        "queste view sono montate e l'inventario non le nomina: {di_troppo:?}\n\
         ...
         `fubmd_features::ogni_view_ufficiale`, e da adesso `view_refresh_masks` \
