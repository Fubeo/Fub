...
const PAZIENZA: Duration = Duration::from_secs(30);

/// Il `.wasm` del plugin che non collabora.
fn ciclo() -> Utf8PathBuf {
    comune::componente("ciclo-wasm", "ciclo_wasm", "")
}
...
fn banco(v: &Vault, wasm: &[Utf8PathBuf]) -> (Host, Subscription) {
    let bundle: Vec<WasmBundle> = wasm
        .iter()
        .map(|w| WasmBundle::da_file(w, Trust::Community).expect("il componente si carica"))
        .collect();
...
fn chiedi(host: &Host, plugin: &str, job: &str) -> JobId {
    host.with_session(None, |s| {
        let mut ws = s.workspace().write().unwrap();
...
fn esito(
    ...
    while inizio.elapsed() < PAZIENZA {
        if let Ok(notice) = eventi.recv_timeout(Duration::from_millis(50)) {
            if let Event::JobDone { id, result, .. } = notice.event {
                if id == atteso {
                    return (result, inizio.elapsed());
                }
            }
        }
    }
    panic!("il job {atteso:?} non è mai tornato: o non lo drena nessuno, o nessuno lo ferma");
}

/// Il testo di un errore che ci aspettiamo essere un guasto del componente.
fn caduto(errore: &PluginError) -> String {
    match errore {
        PluginError::Internal(t) => t
            .as_literal()
            .map(str::to_string)
            .unwrap_or_else(|| format!("{errore}")),
        altro => panic!("un trap arriva al contratto come `internal`, non come {altro:?}"),
    }
}
...
#[test]
fn un_componente_che_non_torna_viene_fermato_e_l_host_resta_vivo() {
    let v = Vault::nuovo();
    let (host, eventi) = banco(&v, &[ciclo(), ping()]);

    // 1. Lo stesso componente, un job che torna: i limiti non sono una tassa su
    //    chi si comporta bene.
    let id = chiedi(&host, CICLO, "eco");
    let (esito_eco, quanto) = esito(&eventi, id);
    assert_eq!(
        esito_eco.expect("un job che torna, torna")["eco"],
        serde_json::json!(true)
    );
    assert!(
        quanto < Duration::from_secs(2),
        "un job che torna subito torna subito: {quanto:?}"
    );
    ...
    let id = chiedi(&host, CICLO, "ciclo");
    let (esito_ciclo, quanto) = esito(&eventi, id);
    let errore = esito_ciclo.expect_err("un ciclo infinito non riesce");
    let detto = caduto(&errore);
    ...
    assert!(
        detto.contains("entro il tempo concesso"),
        "la scadenza arriva al contratto detta com'è, non come `interrupt`: {detto}"
    );
    ...
    assert!(
        quanto >= Duration::from_secs(4),
        "il ciclo ha girato fin quasi alla scadenza, non è caduto subito: {quanto:?}"
    );
    assert!(
        quanto < Duration::from_secs(15),
        "la scadenza è ≈5 s e questa non è: {quanto:?}"
    );
    ...
    let id = chiedi(&host, PING, "ping");
    let (esito_ping, quanto) = esito(&eventi, id);
    let valore = esito_ping.expect("il ping non è stato disturbato dai limiti");
    assert_eq!(valore["nota"], "Nota.md");
    assert!(
        valore["caratteri"].as_u64().unwrap() > 0,
        "il ping ha letto davvero attraverso il confine: {valore}"
    );
    assert!(
        quanto < Duration::from_secs(2),
        "il ping risponde subito: il thread del job non è rimasto appeso ({quanto:?})"
    );
    ...
    let id = chiedi(&host, CICLO, "eco");
    let (dopo, _) = esito(&eventi, id);
    let detto = caduto(&dopo.expect_err("un'istanza che ha trappato non risponde più"));
    assert!(
        detto.contains("cannot enter component instance"),
        "il componente fermato non riparte da solo: {detto}"
    );
...
#[test]
fn un_componente_che_divora_memoria_trova_il_tetto() {
    let v = Vault::nuovo();
    let (host, eventi) = banco(&v, &[ciclo()]);

    let id = chiedi(&host, CICLO, "mangia");
    let (risposta, quanto) = esito(&eventi, id);
    let valore = risposta.expect("chi legge il rifiuto di `memory.grow` resta vivo per dirlo");
    let mib = valore["mib"]
        .as_u64()
        .unwrap_or_else(|| panic!("il job dice quanto ha ottenuto: {valore}"));
    ...
    assert!(
        quanto < Duration::from_secs(2),
        "a fermarlo è stato il tetto e non la scadenza: {quanto:?}"
    );
    ...
    let id = chiedi(&host, CICLO, "mangia");
    let (ancora, _) = esito(&eventi, id);
    assert_eq!(
        ancora.expect("il plugin è ancora vivo")["mib"],
        serde_json::json!(0),
