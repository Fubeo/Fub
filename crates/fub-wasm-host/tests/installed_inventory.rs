...
fn component_path(config: &Utf8Path, installation: u64, bytes: &[u8]) -> Utf8PathBuf {
    config.join("wasm-plugins/components").join(format!(
        "{}-{}.wasm",
        installation,
        Revision::of_bytes(bytes).0.trim_start_matches("sha256:")
    ))
}
...
fn install_restart_decisions_and_remove_keep_four_states_separate() {
    ...
    let loaded = store.load(&installed).unwrap();
    assert_eq!(loaded.installed(), &installed);
    assert_eq!(loaded.bytes(), original);
    assert_eq!(loaded.validate().unwrap().trust(), Trust::Community);
    ...
    assert!(persisted["next_installation"].is_string());
    assert!(persisted["plugins"][0]["installation"].is_string());
...
fn concurrent_installers_have_one_winner_without_overwriting_or_retrying() {
    ...
    assert_eq!(
        results
            .iter()
            .filter(|result| matches!(result, Err(InstallError::Conflict)))
            .count(),
        1,
        "{results:?}"
    );
...
#[test]
fn snapshot_and_load_do_not_execute_unapproved_component_bytes() {
    let dir = tempfile::tempdir().unwrap();
    let config = root(&dir);
    let store = InstalledPluginStore::open(config).unwrap();
    let installed = store
        .install(&store.snapshot().unwrap(), &common::ping(""))
        .unwrap();
    let invalid = b"questi byte non sono un componente wasm";
    let changed_digest = Revision::of_bytes(invalid);
    let original_path = component_path(
        config,
        installed.installation,
        &std::fs::read(blobs(config).remove(0)).unwrap(),
    );
    let changed_path = component_path(config, installed.installation, invalid);
    std::fs::remove_file(original_path).unwrap();
    std::fs::write(changed_path, invalid).unwrap();

    let mut persisted: serde_json::Value =
        serde_json::from_slice(&std::fs::read(inventory(config)).unwrap()).unwrap();
    persisted["plugins"][0]["digest"] = changed_digest.0.into();
    std::fs::write(
        inventory(config),
    ...
    let snapshot = store
        .snapshot()
        .expect("il listing legge soltanto metadati");
    let record = &snapshot.plugins()[0];
    let loaded = store
        .load(record)
        .expect("il load verifica i byte senza eseguirli");
    assert_eq!(loaded.bytes(), invalid);
    assert!(matches!(loaded.validate(), Err(InstallError::Component(_))));
}

#[test]
fn unknown_inventory_fields_yield_to_future_schema_and_refuse_current_schema() {
    let dir = tempfile::tempdir().unwrap();
    let config = root(&dir);
    let store = InstalledPluginStore::open(config).unwrap();
    let installed = store
    ...
    let mut persisted: serde_json::Value =
        serde_json::from_slice(&std::fs::read(inventory(config)).unwrap()).unwrap();
    persisted["future_inventory_field"] = true.into();
    persisted["plugins"][0]["future_plugin_field"] = true.into();
    persisted["plugins"][0]["manifest"]["permissions"]["future_permission_field"] = true.into();

    let mut future = persisted.clone();
    future["schema_version"] = 2.into();
    let future_bytes = serde_json::to_vec_pretty(&future).unwrap();
    std::fs::write(inventory(config), &future_bytes).unwrap();
    assert!(matches!(store.snapshot(), Err(InstallError::Schema(2))));
    assert_eq!(
        std::fs::read(inventory(config)).unwrap(),
        future_bytes,
        "lo schema futuro prevale sui campi che questo reader non conosce"
    );

    let bytes = serde_json::to_vec_pretty(&persisted).unwrap();
    std::fs::write(inventory(config), &bytes).unwrap();
    let error = store.snapshot().unwrap_err();
    let InstallError::Invalid(message) = error else {
        panic!("errore inatteso: {error:?}");
    };
    for field in [
        "future_inventory_field",
        "future_plugin_field",
        "future_permission_field",
    ] {
        assert!(message.contains(field), "campo non segnalato: {field}");
    }
    assert!(matches!(
        store.set_enabled(&base, installed.installation, true),
        Err(InstallError::Conflict)
...
#[cfg(unix)]
#[test]
fn exact_component_symlinks_and_hardlinks_are_not_adopted() {
    let dir = tempfile::tempdir().unwrap();
    let config = root(&dir);
    let source = common::ping("");
    let bytes = std::fs::read(&source).unwrap();
    let destination = component_path(config, 1, &bytes);
    std::fs::create_dir_all(destination.parent().unwrap()).unwrap();
    let backing = config.join("same-component.wasm");
    std::fs::write(&backing, &bytes).unwrap();
    let store = InstalledPluginStore::open(config).unwrap();

    std::os::unix::fs::symlink(&backing, &destination).unwrap();
    let symlink_error = store.install(&store.snapshot().unwrap(), &source);
    assert!(
        matches!(
            symlink_error,
            Err(InstallError::Operation {
        ...
        "errore inatteso: {symlink_error:?}"
    );
    assert!(!inventory(config).exists());
    assert_eq!(std::fs::read(&backing).unwrap(), bytes);

    std::fs::remove_file(&destination).unwrap();
    std::fs::hard_link(&backing, &destination).unwrap();
    let hardlink_error = store.install(&store.snapshot().unwrap(), &source);
    assert!(
        matches!(
            hardlink_error,
            Err(InstallError::Operation {
        ...
        "errore inatteso: {hardlink_error:?}"
    );
    assert!(!inventory(config).exists());
    assert_eq!(std::fs::read(&backing).unwrap(), bytes);
}
