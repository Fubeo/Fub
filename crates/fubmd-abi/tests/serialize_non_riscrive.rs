...
//!
//! Il doc di [`FormatProvider::serialize`](fubmd_abi::FormatProvider::serialize)
...
//! stata calcolata — [`EditRequest`](fubmd_abi::edit::EditRequest).
//!
...
//! kernel, in `fubmd-host`, in `fubmd-app` o in una feature ufficiale — perché
...
/// [`HostApi::apply_edit`](fubmd_abi::traits::HostApi::apply_edit).
#[derive(Debug)]
enum Perche {
...
const ALLOWLIST: &[(&str, &str, usize, Perche)] = &[
    ...
    (
        "crates/fubmd-format-markdown/src/lib.rs",
        "serialize::serialize",
        1,
...
const NON_SI_ENTRA: &[&str] = &["target", "node_modules", ".git", ".fubmd"];
...
fn la_garanzia_e_ancora_scritta_nel_contratto() {
    assert!(
        ...
        "in `crates/fubmd-abi/src/format.rs` non c'è più la frase\n  «{LA_GARANZIA}»\n\
...
fn il_cammino_trova_il_contratto() {
    ...
    assert!(
        sorgenti.contains_key("crates/fubmd-abi/src/format.rs"),
        "il cammino dei sorgenti non ha trovato `crates/fubmd-abi/src/format.rs`, che\n\
...
mod serialize;\n\
use fubmd_abi::traits::serialize;\n\
\n\
