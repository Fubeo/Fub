...
//! [`SettingScope::Machine`] scritte in un `.fubmd/settings.json` si **ignorano**,
...
/// (`fubmd_host::config_dir`). Un terzo strato di merge avrebbe voluto dire un
...
pub enum SettingScope {
    /// Del vault: sta in `.fubmd/settings.json`, **viaggia** col vault, ed è
...
///
/// **Non tagged**: un `.fubmd/settings.json` si apre con un editor di testo, si
