...
//! aprire. Ma **crearne** uno nuovo così è un'altra cosa: è FubMD che scrive un
...
//!   FubMD creerebbe una nota che FubMD non vede. Per questo
...
//!   contratto — `fubmd-abi` non sa dove sia la radice e non deve saperlo. Il
...
pub enum NameFault {
    ...
    /// filesystem ma di FubMD: la scansione salta ogni nome che comincia col
...
/// stessa chiave, quindi per FubMD `Café` composto e `Café` decomposto sono lo
...
mod tests {
    ...
    fn il_punto_in_coda_e_quello_in_testa_sono_due_guasti_diversi() {
        ...
        assert_eq!(faults(".fubmd/roba.md", Naming::New), Some("hidden"));
