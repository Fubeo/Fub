...
| [decisione 0021](../../docs/decisions/0021-il-confine.md) | **`host-api` divisa in nove interfacce** (§7.1): ventiquattro funzioni cambiano nome qualificato (`host-api.read-document` → `host-vault-read.read-document`) e il record `trash-entry` si sposta. È il ritaglio più largo fatto finora, e l'ultimo che riguarda l'`host-api`: dopo il freeze una funzione non si sposta più da un'interfaccia all'altra. Ciò che compra non è ordine — è che il permesso di scrivere possa **non essere importato**, cioè che a M5 il rifiuto sia l'assenza della funzione invece di una risposta a runtime. Il `plugin-world` importa le nove famiglie una per una |
